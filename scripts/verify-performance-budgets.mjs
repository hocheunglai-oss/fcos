...
assertBudget(dispatcherLines <= budgets.server.universalDispatcherLines, `Universal dispatcher has ${dispatcherLines} lines (budget ${budgets.server.universalDispatcherLines}).`);
...
assertBudget(notificationSource.includes("client.rpc('load_work_notification_snapshot'"), 'Notification list does not use the one-request database snapshot.');
assertBudget((notificationSource.match(/listSpecialTermApprovalQueue\(/g) || []).length === 1, 'Notification list Salesforce approval queue request count changed.');
assertBudget((notificationSource.match(/listSpecialTermClauseConsolidations\(/g) || []).length === 1, 'Notification list Salesforce consolidation request count changed.');
...
assertBudget(backgroundHandler.includes("folders: ['inbox']"), 'Foreground Email Router synchronization is no longer inbox-only.');
assertBudget(backgroundHandler.includes('maxPages: 1'), 'Foreground Email Router synchronization page budget changed.');

const functionRoot = path.join(root, '.vercel/output/functions/api');
if (await exists(functionRoot)) {
  const universal = path.join(functionRoot, 'functions/[name].func');
  if (await exists(universal)) {
    const size = await directoryKilobytes(universal);
    assertBudget(size <= budgets.server.universalFunctionKilobytes, `Universal Vercel function is ${size} KB (budget ${budgets.server.universalFunctionKilobytes} KB).`);
  }
  for (const name of ['work-notifications.func', 'email-router-background-sync.func']) {
    const target = path.join(functionRoot, name);
    if (!(await exists(target))) continue;
    const size = await directoryKilobytes(target);
    assertBudget(size <= budgets.server.dedicatedFunctionKilobytes, `${name} is ${size} KB (budget ${budgets.server.dedicatedFunctionKilobytes} KB).`);
  }
}

if (failures.length) {
  for (const failure of failures) process.stderr.write(`[performance budget] ${failure}\n`);
  process.exit(1);
}

process.stdout.write(`Performance budgets passed (${dispatcherLines} dispatcher lines).\n`);
