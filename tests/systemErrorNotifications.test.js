...
test('system error storage is service-only and integrated into unified notifications', async () => {
  ...
  assert.match(notifications, /systemIncidentPresentation\(row.handler\)/);
  assert.match(notifications, /retryAvailable: Boolean\(row\.link\)/);
  assert.match(notifications, /systemIncidentPresentation/);
  assert.match(notificationUi, /Diagnostic reference:/);
  assert.match(notificationUi, /Outcome:/);
  ...
  const recovery = await readFile(new URL('../api/_systemIncidentRecovery.js', import.meta.url), 'utf8');
  assert.match(handler, /createSystemIncidentVerifier/);
  assert.match(recovery, /async function systemErrorVerify/);
  assert.match(recovery, /case 'specialTermsWorkspace'/);
  assert.match(recovery, /case 'hedgeDeskSalesforceMapping'/);
  assert.match(recovery, /case 'hedgeMarkets'/);
  assert.match(recovery, /hedgeMarkets\(\{ action: 'snapshot' \}, req, context\)/);
  assert.match(recovery, /case 'emailRouterMaintenanceCron'/);
  assert.match(recovery, /EMAIL_ROUTER_SYNCHRONIZATION_STALE/);
  assert.match(recovery, /expectedFolders = \['inbox', 'sentitems', 'archive'\]/);
  assert.match(recovery, /case 'salesforceQuery'/);
  assert.match(recovery, /resolveSystemErrorIncident\(context\.client, incidentSignature, verifiedThrough\)/);
