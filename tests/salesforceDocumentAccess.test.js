...
test('all roles require explicit scope and actual document linkage', async () => {
  const h = harness({ query: async () => [] });
  await assert.rejects(download({ ...h.input, interoffice: false, stemId: null }, h.deps));
...
test('dispatcher retains verified context and listing/download share the same live graph', async () => {
  ...
  assert.match(source, /loadScope: \(stemId\) => loadStemDocumentScope/ );
  assert.match(source, /loadStemDocumentScope\(body, accessContext\)/);
  assert.match(source, /queryRows: \(soql\) => queryRows\(soql, \{ limit: 2000, softFail: false \}\)/);
});
