...
test('FCOS Updates uses the shared Microsoft Graph application and no workflow sender', () => {
...
test('FCOS Updates fails closed on partial Microsoft Graph application configuration', () => {
  const partial = fcosUpdateMailConfig({
    FCOS_MICROSOFT_TENANT_ID: 'tenant-id',
  });
  assert.match(partial.configurationIssue, /Complete the Microsoft Graph/);
});

test('FCOS Updates treats Microsoft Graph mailbox authorization as a sender-wide failure', () => {
  const authorizationError = Object.assign(new Error('Microsoft Graph mailbox authorization failed.'), {
    mailDeliveryGlobal: true,
  });
  assert.equal(
    fcosUpdateSenderFailureMessage(authorizationError),
...
test('Settings manages Graph mailboxes and purpose assignments', async () => {
  ...
  assert.match(settingsSource, /Microsoft Graph Email Senders/);
  assert.match(settingsSource, /Purpose assignments/);
  ...
  assert.match(handlerSource, /configuredEmailSenderStatus\(context\.client/);
...
test('FCOS update workflow protects saved revisions and interrupted delivery', async () => {
  ...
  assert.match(server, /sender:[\s\S]*senderResult\.emailAddress/);
  assert.match(server, /sendGraphPurposeMail/);
  assert.match(server, /purposeKey: 'fcos_updates'/);
  assert.doesNotMatch(server, /FCOS_UPDATE_FROM_EMAIL/);
  assert.doesNotMatch(server, /createSmtpTransport/);
