-- Separate the reviewed pre-write fingerprint from the authoritative
-- post-write fingerprint. A financial edit legitimately changes the source
-- fingerprint between review and the Salesforce confirmation write.
...
begin
  ...
  for v_requested in select value from jsonb_array_elements(p_sides) loop
    ...
    if v_side not in ('cost', 'buyer_charge') or v_fingerprint is null or v_expected_fingerprint is null
      ...
      raise exception 'Every confirmed Variable Charges side requires fingerprints, row review, and note.' using errcode = '22023';
    end if;
    select * into v_state
    ...
    if v_state.source_fingerprint <> v_expected_fingerprint then raise exception 'Salesforce source data changed after this side was opened.' using errcode = '40001'; end if;
