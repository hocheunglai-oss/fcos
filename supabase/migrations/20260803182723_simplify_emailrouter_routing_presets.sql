...
begin
  if exists (
    ...
    raise exception 'Duplicate Email Router preset names must be resolved before this migration';
  end if;
end;
$$;

create unique index if not exists emailrouter_routing_presets_display_name_key
on emailrouter.routing_presets (lower(btrim(display_name)));

comment on column emailrouter.routing_presets.preset_key is
'Legacy import identity retained for migration compatibility; FCOS users identify presets by unique display name.';
...
begin
  ...
  if char_length(btrim(coalesce(p_operation->>'displayName', ''))) not between 1 and 255
    ...
    raise exception 'A unique preset name and at least one recipient are required';
  end if;
