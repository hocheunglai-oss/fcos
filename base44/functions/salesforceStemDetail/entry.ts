...
Deno.serve(async (req) => {
  try {
    ...
    const [res, lineItems, extraCosts, buyerBrokers] = await Promise.all([
      ...
      sfQuery(accessToken, `SELECT Id, Name, Description__c, Supplier_Name__c, Quantity__c, Unit_Price__c, Unit_Cost__c, Line_Total__c, Line_Total_Buy__c, Payment_Term__c FROM STEM_Extra_Cost__c WHERE STEM__c = '${actualStemId}' ORDER BY CreatedDate ASC`),
