...
- All four non-secret Preview flags are scoped to this release branch: client OIDC, server federation, pinned issuer and read-only CI. Existing Preview secrets and Salesforce-write disablement are retained; no Production secrets were copied.
...
- Hosted Preview-configured Vercel build and strict client/server bundle budgets pass.
...
1. Review/install the separate narrow `codex/ci-bootstrap-20260908` PR on main, then protect main with workflow-owner review. It removes the legacy credential-bearing PR job; it deliberately does not install the candidate-evidence consumer before a trusted run can exist.
