...
export const PAGE_METHODOLOGIES = {
  Markets: {
    ...
    steps: [
      ...
      "The selected report date applies consistently to every Markets tab. The floating Market Pulse is intentionally latest-only, so it remains an operational current snapshot rather than a historical report view.",
      "Historical month estimates are reconstructed from the currently stored MOPS rows dated on or before the selected report date using the same approved current-month estimator. Later corrections may therefore differ from the record originally available on that date. The board exposes actual, estimated and carried counts plus represented publication days; represented days, including carried days, are not a source-record sample count.",
      ...
      "Delivered-price history is retrieved through every available page for the selected range. Premium or discount is calculated only against the exact-date MOPS observation; no cross-date benchmark is substituted.",
  ...
  "Physical trades": {
    ...
    steps: [
      ...
      "A final Paper Hedge result is allocated across its linked Physical Trades by normalized midpoint quantity. ICE uses gross hedge P&L because broker, exchange, clearing, and settlement fees are billed separately. FCBS deducts the direct FCBS venue charge because FCBS bills that cost directly to us.",
      "For each venue, a net hedge gain becomes a negative Salesforce STEM cost and a net hedge loss becomes a positive STEM cost. The review shows gross P&L, included direct costs, net result, and proposed Salesforce cost before any write.",
