...
function ReportImportDrawer({ open, onClose, onImported }) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Import market report" description="FCOS stores the governed trading series plus the report's structured product names, codes and prices. PDF bytes and report text are not retained." width="large" footer={<><Button onClick={onClose} disabled={busy}>Cancel</Button>{preview ? <Button variant="primary" onClick={save} disabled={busy || !entitlementConfirmed || importBlockers.length > 0}>{busy ? 'Importing...' : `Import ${preview.libraryObservationCount || preview.observationCount} structured values`}</Button> : <Button variant="primary" onClick={prepare} disabled={busy || !file}>{busy ? 'Reading...' : 'Review values'}</Button>}</>}>
      {error ? <InlineError error={error} /> : null}
      ...
      {preview ? <section className="app-form-section">
        ...
        <div className="market-import-summary"><strong>{preview.libraryObservationCount || 0} structured report values across {preview.libraryProductCodeCount || 0} product codes</strong><span>{preview.observationCount} governed trading values feed MOPS, curves and delivered-price logic.</span><span>{preview.publishedNaSymbols?.length ? `${preview.publishedNaSymbols.length} governed symbols explicitly published N/A: ${preview.publishedNaSymbols.map((symbol) => marketSymbolLabel(symbol)).join(', ')}` : 'No governed symbol is explicitly published N/A.'}</span><span>{preview.missingSymbols.length ? `${preview.missingSymbols.length} governed symbols not detected: ${preview.missingSymbols.map((symbol) => marketSymbolLabel(symbol)).join(', ')}` : 'No governed symbol is genuinely missing.'}</span></div>
...
function MarketToolsDrawer({ open, onClose, onImport, data, settings, canManageMarketData, canManageAlertRules, canManageCurveCutover, priceEntity, verifyMonth, marketDataLoaded, marketDataLoading }) {
  return <Drawer open={open} onClose={onClose} title="Market tools" description="Governed report, settlement, curve, and alert administration. Daily market reading remains unchanged." width="xl" footer={<Button onClick={onClose}>Close</Button>}>
    ...
    <div className="market-tools-stack">
      ...
      {canManageMarketData && marketDataLoaded ? <details className="market-tools-section"><summary>Settlement MOPS control</summary><div className="market-tools-embedded-workspace"><div className="app-callout app-callout--neutral">For a daily text bulletin, choose <strong>Add MOPS price</strong>, paste it under <strong>Published MOPS message</strong>, then review the parsed S380, S0.5 and SGO values before saving.</div><MarketsView embedded showLegacyForward={false} data={data} settings={settings} readOnly={false} priceEntity={priceEntity} verifyMonth={verifyMonth} /></div></details> : null}
