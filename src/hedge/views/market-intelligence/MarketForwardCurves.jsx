  ...
  return (
    <div className="market-intelligence-stack" data-testid="market-forward-curves">
      ...
      <Panel>
        ...
        {signals.length ? <div className="market-context-units">{['USD/MT', 'USD/BBL'].map((unit) => { const unitSignals = signals.filter((signal) => String(signal.unit || 'USD/MT').toUpperCase() === unit); return unitSignals.length ? <section key={unit}><div className="market-context-units__title">{unit}</div><div className="market-context-grid">{unitSignals.map((signal, index) => <article key={signal.id || `${signal.key || signal.type || signal.label}:${index}`}><Scale size={17} /><span>{signal.label || signal.type || signal.key}</span><strong>{formatPrice(signal.value, unit)}</strong><small>{[signal.contractMonth ? monthLabel(signal.contractMonth) : signal.date ? formatDate(signal.date) : null, signal.tenor, signal.basis, signal.sourceSymbol].filter(Boolean).join(' · ')}</small></article>)}</div></section> : null; })}</div> : !contextPanels.length ? <div className="market-empty-inline"><CalendarRange size={20} /><div><strong>No cross-market context loaded</strong><span>Missing context is not derived from report commentary.</span></div></div> : null}
      </Panel>
