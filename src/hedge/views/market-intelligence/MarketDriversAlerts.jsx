    ...
    {catalogueResponse ? <>
      ...
      <div className="market-library-builder">
        ...
        <div className="market-library-question">
          ...
          <Field label="AI model"><Select value={modelId} onChange={(event) => setModelId(event.target.value)}>{array(catalogueResponse.models).map((model) => <option key={model.id} value={model.id}>{model.label}{model.recommended ? ' · Recommended' : ''}{model.costTier ? ` · ${model.costTier}` : ''}</option>)}</Select></Field>
