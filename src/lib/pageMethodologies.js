...
export const EMAIL_ROUTER_METHODOLOGY = {
  ...
  sections: [
    ...
    { title: 'Background synchronization', body: 'While FCOS is open, the shared mailbox synchronizes every 30 seconds from every FCOS page, not only Email Router. A server-side claim prevents multiple users or browser tabs from duplicating the same Graph synchronization. Microsoft Graph notifications continue to synchronize mailbox changes when no user is viewing FCOS, with scheduled maintenance as recovery.' },
