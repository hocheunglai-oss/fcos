...
export const EMAIL_ROUTER_METHODOLOGY = {
  ...
  sections: [
    ...
    { title: 'Background synchronization', body: 'While FCOS is open, the shared mailbox synchronizes every 30 seconds from every FCOS page, not only Email Router. A server-side claim prevents multiple users or browser tabs from duplicating the same Graph synchronization, while every open Email Router list refreshes after each successful heartbeat even when another session completed the shared sync. Microsoft Graph notifications continue to synchronize mailbox changes when no user is viewing FCOS, with scheduled maintenance as recovery.' },
    ...
    { title: 'Content protection', body: 'Learning stores only protected sender and subject fingerprints, broad categories, attachment profiles, approved directory references, and routing choices. It never stores message bodies, MIME, recipient arrays, attachment content, or manual email addresses. HTML is sanitized and embedded images use authorized temporary endpoints.' },
  ],
};
