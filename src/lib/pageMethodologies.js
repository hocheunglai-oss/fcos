...
export const SETTINGS_METHODOLOGIES = {
  ...
  health: {
    ...
    sections: [
      ...
      { title: 'Connection order', body: 'The Connection Checklist requires an approved CLI and version first, then exact non-secret account, team, project, repository, or organization identity plus live permission probes, then target-locked CLI use. DEVEE is the only Salesforce development/source environment. Each change is deployed and verified in DEVEE, mirrored byte-for-byte to `ivanyk20/fcbhk`, promoted to QAT, and then promoted to Production—never in another order. Shared publication requires a fresh successful DEVEE deployment proof for the exact source-tree hash and the isolated `vincelessxai` GitHub identity; QAT and Production are never independent mirror sources. Mirror branches use an exact remote-head lease, and GitHub must map both commit author and committer to the pinned `vincelessxai` account ID. A provider connector or API remains preferred when capable; Chrome is allowed only for blocked authentication, using Otto for FCOS or `vincexai` for the shared Salesforce repository, followed by CLI reverification.' },
