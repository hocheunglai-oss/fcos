...
export const SETTINGS_METHODOLOGIES = {
  ...
  finance: {
    ...
    sections: [
      ...
      body: 'The Dashboard uses Actual/365 and actual Salesforce cash allocations to calculate the daily funded balance for each STEM and currency. Funding is cumulative supplier cash paid less cumulative buyer cash received; advances stay in the running balance and only a positive balance bears interest. Each actual supplier remittance also incurs the saved UBS or DBS USD fee. Shared remittance fees are allocated proportionally across every cash allocation in the remittance, including STEMs outside the active filters. A missing bank or required USD conversion withholds the affected bank charge and EBIT; FCOS does not infer either. For STEMs actually delivered in 2026, pre-2026 cash allocations are included from their original dates when all evidence checks pass. Expected dates alone do not qualify. Missing or unresolved evidence withholds the affected finance result instead of recording zero.',
      },
      {
