...
export const DASHBOARD_METHODOLOGY = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: 'Credit Statements include only STEMs whose Salesforce Account lookup is the selected active Account; supplier line items, extra costs, brokers, and inactive duplicate Accounts never widen membership. Current signed QLIK receivable balances reconcile independently to individual and ultimate-GROUP used-credit fields within one currency unit. If the selected Account credit fields conflict, FCOS may use exactly one compatible, uniquely reconciling same-name lineage snapshot and duplicate-record Created Date window, and displays the fallback CL Key and window. Older nonzero balances stay visible as evidence but are labelled outside the current credit projection. Square step lines show remaining individual and GROUP exposure after each evidenced release. Exposure without a reliable date remains in the final plateau, with separate selected-Account and GROUP amounts and counts, instead of receiving an invented release date.',
    },
    {
      title: 'AI Search',
