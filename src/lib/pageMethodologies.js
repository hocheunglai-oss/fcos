...
export const SETTINGS_METHODOLOGIES = {
  ...
  health: {
    ...
    sections: [
      ...
      { title: 'Connection order', body: 'The Connection Checklist requires CLI availability first, then an exact non-secret account, team, project, repository, or organization match, then CLI use. The shared FCOS wrapper repeats this check before every delegated command. A provider connector or API remains preferred when capable; the Otto Chrome profile is unlocked only to resolve an explicit CLI authentication failure, after which identity must be reverified in the CLI.' },
      { title: 'Durable isolation', body: 'GitHub and Vercel use ignored repo-local configuration roots with their supported secure session storage. The pinned Supabase CLI uses an ignored 0600 token file because its host credential store cannot be reliably isolated. Salesforce does not expose an alternate authorization home, so FCOS retains its protected host session while isolating the target-org pin and revalidating the production organization live.' },
      { title: 'Secret-safe records', body: 'Checklist records remain in the current browser and accept only controlled states, timestamps, the approved browser profile name, and fixed non-secret FCOS target identifiers. The local verifier writes the same restricted status shape to an ignored file; neither path accepts CLI output, tokens, keys, passwords, or credential material.' },
