...
export const EMAIL_ROUTER_METHODOLOGY = {
  ...
  sections: [
    ...
    { title: 'Background synchronization', body: 'While FCOS is open, the shared mailbox synchronizes every 30 seconds from every FCOS page, not only Email Router. A server-side claim prevents multiple users or browser tabs from duplicating the same Graph synchronization, while every open Email Router list refreshes after each successful heartbeat even when another session completed the shared sync. The page shows its last successful synchronization and provides a catch-up action. Microsoft Graph notifications continue to synchronize mailbox changes when no user is viewing FCOS, with scheduled maintenance as recovery.' },
    ...
    { title: 'Routing leave and snapshots', body: 'Routing Leave records operational availability, not HR approval. Every authorized Email Router user manages their own periods; Administrators and the active General Manager may manage periods for any active FCOS user. A reviewed preset route is held by a server-signed snapshot for up to 60 minutes unless directory or preset configuration changes. FCOS warns, but does not block, when a chosen recipient is currently on leave.' },
    { title: 'Route Message', body: 'The fixed Route Message panel handles Forward and Redirect with ordered To, Cc, and Bcc recipients. Forward may include an introduction. Redirect shows the reviewed To and Cc recipients, keeps Bcc private, and includes the original From, To, and Cc with the original message. Reply All restores the original sender and visible original To/Cc recipients except the shared mailbox, together with the visible redirect recipients. Original Bcc recipients are never restored or exposed. Presets remain editable after selection, and any manual recipient amendment clears the preset label.' },
    { title: 'Post-action filing', body: 'Redirect defaults to Archive and Forward defaults to leaving the source in its current folder. Users may instead select an Administrator-approved Microsoft 365 folder. The selected destination is snapshotted when the action is reserved, so a later folder-setting change cannot alter reviewed work.' },
    { title: 'Submission safety', body: 'Graph submission is durable: Reserved, Draft Created, Submitted, Confirmed, Failed, or Uncertain. A 202 response is not treated as delivery. FCOS keeps the source visible until Sent Items confirms the outgoing draft, then files it. A filing failure never resends the email; an allowed retry moves only the source message. Batch routing is limited to ten reviewed messages, records each outcome independently, and never automatically retries an uncertain result.' },
    { title: 'Advisor learning', body: 'The Advisor learns company-wide only from confirmed human Forward and Redirect outcomes. It may preselect the action, recipients, or folder only when at least three similar outcomes agree and that component confidence is above 60%. Conflicting or insufficient history remains advisory. Administrators may disable learning or forget an incorrect aggregate pattern.' },
    { title: 'Content protection', body: 'Learning stores only protected sender and subject fingerprints, broad categories, attachment profiles, approved directory references, and routing choices. It never stores message bodies, MIME, recipient arrays, attachment content, or manual email addresses. HTML is sanitized; embedded images use authorized temporary endpoints; likely tracking pixels remain blocked; and remote-image trust stores only a sender domain in the current browser. PDF text extraction is transient and searchable but is not retained by FCOS.' },
    { title: 'Speed and operations', body: 'Message details are prefetched on row focus or hover, cached briefly in memory, and obsolete detail requests are cancelled. Keyboard shortcuts support navigation, search, filing, and routing focus without overriding text entry. The Operations view reports redacted Graph subscription, delta-folder, alert, route-outcome, synchronization, and client-timing health; it stores no message or recipient content.' },
  ],
};
...
export const DASHBOARD_METHODOLOGY = {
  ...
  sections: [
    ...
    {
      title: 'EBIT and finance deductions',
      body: 'Gross Profit remains the default on every fresh Dashboard load. Enabling EBIT changes only that KPI and subtracts interest finance cost and combined bank charges from gross profit separately for each currency. FCOS applies the current company annual financing rate to every selected historical period using Actual/365: interest finance cost is the sum of each positive daily funded balance multiplied by the annual rate and divided by 365. The funded balance is cumulative actual supplier cash payments less cumulative actual buyer cash receipts. Buyer advances remain in the running balance, only the interest-bearing amount is floored at zero, partial payments change the balance on their actual dates, and a same-day supplier payment and buyer receipt incurs no interest. Salesforce cash allocations include identifiable signed refunds and reversals and exclude remittance headers, volume discounts, write-offs, commissions, Bank_Charge rows, and other non-cash adjustments from the interest cash balance. FCOS preserves the original actual prepayment Payment ID when a later invoice link appears, so that link cannot count the cash twice. A synthetic Is_Deposit allocation without the original actual funding-date linkage makes finance unavailable; FCOS never estimates the funding date or finance cost. Bank Charge combines supplier-remittance fees with recorded buyer-receipt Bank_Charge amounts. Each actual supplier remittance incurs the current USD fee for its selected company bank. A fee shared across several cash allocations is apportioned by their full remitted amounts, including allocations to STEMs outside the current Dashboard filters, so filtering cannot reassign the fee. An unknown bank or missing USD-to-STEM-currency evidence makes the affected supplier charge and EBIT unavailable; FCOS never guesses a bank or exchange rate. Buyer receipts use only their recorded Bank_Charge amounts: no receipt default is inferred, and signed refund charges net the combined total. Receipt charges do not change the buyer cash amount or the interest cash-balance timeline. A settled STEM stops at its final actual buyer receipt; open funding accrues through the current Hong Kong date. The card shows the current rate and supplier fee settings, shared revision, calculation date, both deductions, and missing-evidence count. STEMs actually delivered in 2026 may use pre-2026 cash allocations, retaining the full funding period and all reconciliation checks. Expected delivery dates alone do not qualify for this exception. Incomplete or unresolved settlement or bank-charge evidence excludes that STEM from finance results. Partial EBIT shows only verified STEMs, with their gross profit, interest finance cost, combined bank charges and coverage count; the full selection gross profit remains visible for context. If no STEM can be verified, the card shows Gross profit (before finance) and identifies EBIT as unavailable. Unknown costs are never treated as zero.',
    },
    {
    ...
    },
    {
      ...
      body: 'Export XLS uses the ordinary Dashboard selection and retrieves every server page with the active filters, submitted text search, and sort. Progress and cancellation remain visible, and FCOS does not silently download a partial result. The Excel-compatible .xls file is built and downloaded locally, keeps text literal and currency totals separate, splits oversized row sets across worksheets, and includes a Scope sheet with the filters, row count, generation time, and calculation notes. When EBIT is enabled, the export includes finance cost, EBIT, and evidence status while holding one rate revision and calculation date across every page. The Scope sheet separates full gross profit from verified gross profit, finance cost and EBIT, and records the number of verified STEMs. AI-result mode must be cleared before exporting.',
    },
    {
    ...
    },
    {
      ...
      body: 'Credit Statements default to open buyer-leg exposure and include only STEMs whose Salesforce Account lookup is the selected active Account and whose actual Delivery Date, or Expected Delivery Date when actual is unavailable, is on or after 1 January 2026. This is an absolute Statement Evidence and used-credit bridge boundary: Open, Open + 12 months settled, and All history can never return an earlier delivery. A GROUP Insight shows every active hierarchy Account as a selectable label; the selected subset alone contributes STEMs, evidence, and combined exposure. The credit authority is resolved independently from the complete exact Salesforce GROUP hierarchy, so child selection never changes the full GROUP limit, used credit, or effective availability. Conflicting authority snapshots fail closed. Buyer exposure remains exact Account-ID QLIK receivables. A separate reconciliation bridge explains Salesforce GROUP used credit as buyer receivables plus included supplier Cashflow gross exposure; it de-duplicates linked Supplier Invoices or child records, excludes financial rows whose amounts are all zero, keeps currencies separate, and displays any unexplained residual instead of folding supplier exposure into buyer risk. Salesforce CL Category controls access: Individual uses only its Account limit, Group shares remaining GROUP capacity without an individual cap, and Special shares GROUP capacity subject to its special Account cap. CL_Available_Credit__c and the used-credit fields remain authoritative; FCOS shows its calculated category value only when the difference exceeds one currency unit. For an un-invoiced quantity-range STEM, the solid current-exposure line follows Salesforce QLIK midpoint quantity. FCOS calculates a separate minimum-to-maximum quantity-risk band anchored to that authoritative midpoint, and keeps the band optional and off by default. Once delivered BDN quantity is nonzero, the range collapses to actual delivered quantity. The conservative Expected Invoice Amount remains a distinct maximum-quantity estimate and is not relabelled as current Salesforce exposure. Incomplete range evidence hides only the band, not the Salesforce midpoint line. If the selected Account credit fields conflict, FCOS may use exactly one compatible, uniquely reconciling same-name lineage snapshot and duplicate-record Created Date window. Square step lines show remaining Account and GROUP exposure after actual payments, scheduled payments, or modeled receipt dates. The model reuses Cashflow payment evidence with exact Buyer, exact GROUP, then global fallback and offers Typical P50, Cautious P75, and Severe P90 recency-weighted delays inside the chart. Cautious is the company default; Administrators and the active General Manager may change it, while other users may preview another level without saving. Applicable positive limits appear in a compact legend rather than over the chart. Exposure without a contractual date remains in the final plateau. Diagnostic assumptions are available on demand and hidden by default. Statement rows can be selected and copied in the Payment Reminder detail format without sending an email. Complete live final Buyer Invoices contribute to currency-separated totals; a STEM without one remains highlighted as Not Issued in FCOS, while copied details use emphasized Expected Invoice Amount and Expected Due Date labels instead of repeating that status.',
    },
    {
      title: 'Supplier Credit Statement and payable forecast',
      body: 'Supplier Credit Statement interprets supplier credit as FCOS payable exposure. It defaults to the exact active supplier Account and open exposure; Include GROUP is optional and adds only active descendants of the ultimate Salesforce GROUP. In a GROUP Insight, the same Account label selection is shared across Both, Buyer, and Supplier views so receivable, payable, and combined exposure always use the identical subset. Issued rows use the live Supplier Invoice actual Supplier lookup, invoice amount, payable balance, due dates, and non-voided payments. A linked exact-supplier child is used only when the invoice has no conflicting actual supplier; conflicts are excluded and flagged. Uninvoiced estimates include only non-cancelled, unlinked exact-supplier line items and extra costs. Quantity precedence is delivered BDN, ordered range maximum, then ordinary ordered quantity. Fixed and per-unit buy-side costs follow Salesforce evidence, and missing prices, quantities, UOMs, or ambiguous invoice linkage suppress the affected currency total rather than becoming zero. Square step lines descend on exact Cashflow, partial-invoice, Supplier Invoice due, child Cashflow, or deterministic delivery/payment-term evidence, with unknown-date obligations retained in the final plateau. Rows may be selected and copied with currency-separated total payable exposure; estimates are labelled Expected and maximum-range rows are labelled Basis Max Qty.',
    },
    {
      title: 'AI Search',
...
export const SETTINGS_METHODOLOGIES = {
  ...
  finance: {
    ...
    sections: [
      {
        title: 'Company finance settings',
        body: 'Annual financing rate (%) and the UBS and DBS supplier-remittance fees are company settings used by Dashboard EBIT. The rate accepts 0 to 100 and each USD fee accepts 0 to 1,000,000, all with at most two decimal places. Initial defaults are 5.00%, UBS USD 10, and DBS USD 15. Current settings apply to historical Dashboard selections, so a saved change recalculates them rather than preserving values from the selected period.',
      },
      {
        title: 'Calculation effect',
      body: 'The Dashboard uses Actual/365 and actual Salesforce cash allocations to calculate the daily funded balance for each STEM and currency. Funding is cumulative supplier cash paid less cumulative buyer cash received; advances stay in the running balance and only a positive balance bears interest. Bank Charge combines two sources. Each actual supplier remittance incurs the saved UBS or DBS USD fee, allocated proportionally across every cash allocation in the remittance, including STEMs outside the active filters. A missing bank or required USD conversion withholds the affected supplier charge and EBIT; FCOS does not infer either. Actual buyer receipts add only recorded Bank_Charge amounts; a receipt has no default charge, signed refunds net their recorded charges, and these charges never change the interest cash balance. For STEMs actually delivered in 2026, pre-2026 cash allocations are included from their original dates when all evidence checks pass. Expected dates alone do not qualify. Missing or unresolved evidence withholds the affected finance result instead of recording zero.',
      },
      {
        title: 'Permissions, revision, and refresh',
        body: 'Existing Finance settings managers, Administrators, and the General Manager may save the rate and both bank fees. Dashboard users without management access can read them. The initial company defaults are UBS USD 10 and DBS USD 15 per actual supplier remittance; current values also apply to historical selections. Every save validates bank fees from USD 0 to 1,000,000 with at most two decimal places, writes all three values under the revision that was loaded, rejects a stale concurrent edit, appends server audit history, and invalidates Dashboard finance results so one calculation cannot mix revisions.',
      },
    ],
  },
  ai: {
