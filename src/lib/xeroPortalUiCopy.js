...
const CONTACTS_COPY = [
  ["kpis", [
    ["active", 'Non-archived Xero', '未封存 Xero'],
  ...
  ["title", 'Contact Cleanup & Sync', '聯絡人整理及同步'],
  ...
  ["search", 'Search Xero, Salesforce, CL Key', '搜尋 Xero、Salesforce、CL Key'],
  ...
  ["selectVisible", 'Select visible eligible', '選取畫面中合資格項目'],
  ...
  ["identity", [
    ["review", 'Review Xero-only identity', '檢閱 Xero 獨有身分'],
  ...
  ["repair", [
    ...
    ["reviewed", 'I reviewed these missing Salesforce contacts', '我已檢閱這些缺少的 Salesforce 聯絡人'],
...
const REASONS_COPY = [
  ["blank-salesforce-key", 'Missing Salesforce CL Key', '缺少 Salesforce CL Key'],
...
const FINANCIAL_COPY = [
  ["loadingTitle", 'Loading accounting controls', '正在載入會計控制項'],
  ...
  ["reconciliationDescriptions", [
    ["not_checked", 'Run Check everything to compare Salesforce and Xero.', '按「檢查全部」建立完整 Salesforce 及 Xero 控制總數。'],
  ...
  ["productMappingsLabel", 'Xero product mappings', 'Xero 產品對應'],
  ...
  ["documentTableLabel", 'Xero financial cutover classifications', 'Xero 財務切換分類'],
  ...
  ["paymentsTitle", 'Exact payment allocations', '精確付款分配'],
  ...
  ["financeReviewedPayments", 'Finance reviewed payments', '財務部已覆核付款'],
  ...
  ["fileDiscovery", [
    ["title", 'Attached PDF candidates', '附加 PDF 候選文件'],
  ...
  ["paymentHoldsDescription", 'These records remain unresolved and cannot be selected for automatic linking or payment. Other verified invoice payments can still be reviewed.', '這些紀錄尚未解決，不能選作自動連結或付款。其他已核實的發票付款仍可覆核。'],
  ...
  ["paymentHoldKinds", [
    ["credit_note", 'Credit-note refund', '貸項退款'],
  ...
  ["basis", [
    ["exact_line", 'line coding matched', '明細編碼相符'],
  ...
  ["batchCompleted", 'Xero document batch completed', 'Xero 文件批次已完成'],
  ...
  ["documentKinds", [
    ["buyer_invoice", 'Buyer invoice', '買方發票'],
  ...
  ["actions", [
    ["create_draft", 'Create draft', '建立草稿'],
...
const STATUSDESCRIPTIONS_COPY = [
  ["eligible", 'Safe to apply after review and selection.', '覆核及選取後可安全套用。'],
...
const TOASTS_COPY = [
  ["connected", 'Xero connected', 'Xero 已連線'],
...
const AUTOMATION_COPY = [
  ["title", 'Salesforce Trigger Contact Creation', 'Salesforce 觸發建立聯絡人'],
