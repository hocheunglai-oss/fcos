import { Eye, FileClock, FileText, Monitor } from 'lucide-react';
...
export default function SpecialTermDocumentPreview({ term, detail, revision, unsaved = false, onPreviewPdf }) {
  ...
  return (
    <section className="min-w-0 space-y-3 rounded-lg border border-border bg-muted/20 p-3 sm:p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><strong className="text-sm">A4 document preview</strong><Badge variant={model.isDraft ? 'secondary' : 'default'}>{model.isDraft ? 'Draft preview' : 'Live document'}</Badge></div><p className="mt-1 text-xs text-muted-foreground">Preview of Terms Text. Open the PDF preview to check final page breaks and download.</p></div>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-background p-2">
        <Button type="button" size="sm" variant="outline" onClick={() => onPreviewPdf?.(effectiveMode)} disabled={!onPreviewPdf || (model.isDraft && !draftExportAllowed)}>
          <Eye className="mr-1 h-3.5 w-3.5" />{model.isDraft ? 'Preview draft PDF' : 'Preview live PDF'}
        </Button>
        {model.isDraft && unsaved ? <span className="text-xs text-amber-800">Save this revision before opening its PDF preview.</span> : null}
        {effectiveMode === 'live' ? <span className="text-xs text-muted-foreground">PDF previews use the current saved Terms Text. Confirmation and Nomination remarks are excluded.</span> : <span className="text-xs text-amber-800">Draft PDFs are watermarked and cannot be used as issued documents.</span>}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
        <span>Estimated {pages.length} {pages.length === 1 ? 'page' : 'pages'} · layout preview</span>
