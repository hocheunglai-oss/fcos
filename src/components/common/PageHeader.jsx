...
export default function PageHeader({ icon: Icon, eyebrow, title, description, meta, status, actions, compact = false, sticky = true, inlineMeta = false, className }) {
  ...
  const contextVisible = Boolean(copy.description || (!inlineMeta && meta) || status);
  return <>
    <header className={cn(
      ...
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0">
          ...
          {inlineMeta && meta ? <div className="mt-1 text-xs leading-4">{meta}</div> : null}
        </div>
      </div>
    ...
    {contextVisible && <div className="app-page-header-context mb-5 grid gap-2 border-x border-b border-border/70 bg-card/80 px-4 py-2.5 text-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
      <div className="min-w-0">
        ...
        {!inlineMeta && meta && <div className="mt-1 text-xs leading-4 text-muted-foreground">{meta}</div>}
      </div>
