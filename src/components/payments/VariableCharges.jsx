...
import {
  ...
  isPortClearanceItem as isPortClearanceProductName,
...
function usesHongKongChargeUi(item) {
  return item?.hongKongVariableCharges === true || item?.hong_kong_variable_charges === true;
}

function isPortClearanceItem(item) {
  return usesHongKongChargeUi(item) && isPortClearanceProductName(item);
}

function isHongKongPortClearanceItem(item) {
  return isPortClearanceItem(item);
}

function supplierChargeUiModel(item) {
  const hongKongVariableCharges = usesHongKongChargeUi(item);
  return {
    hongKongVariableCharges,
    portClearance: isPortClearanceItem(item),
    showCurrencySelector: hongKongVariableCharges,
    showHkdPreview: hongKongVariableCharges,
  };
}

function variableChargeUiQuantityLabel(item, quantity, unitOfMeasure) {
  if (isHongKongPortClearanceItem(item)) return variableChargeQuantityLabel(item, quantity, unitOfMeasure);
  if (quantity == null || quantity === '') return 'Not set';
  return `${quantity} ${text(unitOfMeasure) || 'Not set'}`;
}
...
function initialExtraDraft(item) {
  const storedPricingType = item.fixed === true || item.Fixed__c === true || valueOf(item, ['pricingType', 'pricing_type']) === 'fixed' || valueOf(item, ['fixedCost', 'fixed_cost', 'fixedPrice', 'fixed_price'], null) != null ? 'fixed' : 'per_unit';
  const hongKongVariableCharges = usesHongKongChargeUi(item);
  const lightDues = hongKongVariableCharges && text(valueOf(item, ['productName'])).toUpperCase() === 'LIGHT DUES';
  const portClearance = isHongKongPortClearanceItem(item);
  // The application count is evidence, not the Salesforce pricing basis. Store
  // the exact supplier and buyer totals as fixed amounts so cent rounding cannot
  // drift when the buyer pays for one fewer application than the supplier.
  const pricingType = portClearance ? 'fixed' : storedPricingType;
  ...
  const portClearanceCalculation = portClearance && item?.portClearance?.complete === true
    ? item.portClearance
    : null;
  const requiredInputCurrency = hongKongVariableCharges
    ? text(valueOf(item, ['supplierCurrency.requiredInputCurrency']))
    : '';
  const inputCurrency = hongKongVariableCharges
    ? requiredInputCurrency || (statutoryDefault ? 'HKD' : text(valueOf(item, ['supplierCurrency.inputCurrency'])) || 'USD')
    : 'USD';
  const storedSupplierCostUsd = pricingType === 'fixed'
    ? valueOf(item, ['fixedCost', 'fixed_cost', 'Lumpsum_Cost__c'], '')
    : valueOf(item, ['cost', 'unitCost', 'unit_cost', 'Unit_Cost__c'], '');
  ...
  const anchorageBuyerSuggestion = anchorageBuyerDefault?.available === true
    ...
    : null;
  return {
    description: text(valueOf(item, ['description', 'Description__c']))
      || text(valueOf(item, ['productName', 'Product2Id__r.Name', 'name', 'Name']))
      || 'STEM Charge',
    ...
    supplierCost: !hongKongVariableCharges
      ? storedSupplierCostUsd
      : portClearanceCalculation
      ? (inputCurrency === 'HKD' ? portClearanceCalculation.supplierHkd : portClearanceCalculation.supplierTotalUsd)
      : statutoryDefault?.amountHkd ?? valueOf(item, ['supplierCurrency.inputAmount'], null) ?? (pricingType === 'fixed'
      ? valueOf(item, ['fixedCost', 'fixed_cost', 'Lumpsum_Cost__c'], '')
      : valueOf(item, ['cost', 'unitCost', 'unit_cost', 'Unit_Cost__c'], '')),
    buyerPrice: portClearanceCalculation?.buyerTotalUsd
      ?? anchorageBuyerSuggestion
      ?? (lightDues && finiteNumber(storedBuyerPrice) == null ? 0 : storedBuyerPrice),
...
function initialAddDraft(caseRow, supplierId = '', companyRate = null) {
  ...
  const hongKongVariableCharges = usesHongKongChargeUi(caseRow);
  const requiredInputCurrency = hongKongVariableCharges && supplierAccount?.isAgent === true
...
function supplierCostReviewState(row, review = {}, draft = initialExtraDraft(row.item)) {
  const outcome = review.outcome;
  if (row.readOnly || row.sourceType !== 'extra_cost') {
    return { outcome, requiresUpdate: false, valid: outcome === 'correct' };
  }
  const original = initialExtraDraft(row.item);
  if (outcome === 'cancelled') {
    return { outcome, requiresUpdate: false, valid: draft.cancelled === true && !original.cancelled };
  }
  if (!['correct', 'changed'].includes(outcome)) return { outcome, requiresUpdate: false, valid: false };
  const portClearance = isHongKongPortClearanceItem(row.item);
  const supplierFields = (value) => JSON.stringify({
    description: text(value.description),
    pricingType: value.pricingType,
    supplierCost: finiteNumber(value.supplierCost) ?? text(value.supplierCost),
    inputCurrency: value.inputCurrency,
    quantity: portClearance || value.pricingType === 'per_unit' ? finiteNumber(value.quantity) ?? text(value.quantity) : null,
    unitOfMeasure: text(value.unitOfMeasure),
  });
  const fieldsChanged = supplierFields(draft) !== supplierFields(original);
  const requiresUpdate = draft.statutorySupplierDefaultPending === true
    || (portClearance && outcome === 'changed')
    || fieldsChanged;
  const quantity = finiteNumber(draft.quantity);
  const supplierCost = finiteNumber(draft.supplierCost);
  // Blank stored descriptions start with the charge's product name so the
  // required description is visible and usable before a cost amendment.
  const complete = !draft.cancelled && supplierCost != null && supplierCost >= 0
    && (portClearance ? Number.isInteger(quantity) && quantity >= 1
      : (!fieldsChanged && outcome === 'correct') || Boolean(text(draft.description)))
    && (draft.pricingType !== 'per_unit' || (quantity > 0 && Boolean(text(draft.unitOfMeasure))));
  return {
    // Correct finishes review of the current draft. A material edit still uses
    // the server's changed outcome so its write and audit evidence agree.
    outcome: requiresUpdate ? 'changed' : outcome,
    requiresUpdate,
    valid: requiresUpdate ? complete : outcome === 'correct' && !draft.cancelled,
  };
}
...
export default function VariableCharges({ onOpenStem = null, initialStemId = '', onTaskOpen = null, onTaskClose = null }) {
  ...
  const loadDetail = useCallback(async (stemId, { force = false } = {}) => {
    ...
    const nextDetail = {
      ...rawDetail,
      case: { ...(rawDetail.case || {}), hongKongVariableCharges },
  ...
  const verifySupplierStage = async (requirement) => {
    ...
    for (const row of stageRows) {
      ...
      if (!supplierCostReviewState(row, review, extraDrafts[row.sourceId]).valid) { setSaveError(`Complete the cost details for ${itemLabel(row)}, or mark an unchanged cost Correct.`); return; }
    }
    ...
    (detail?.extraCosts || []).forEach((item, index) => {
      ...
      if (matchingRow && supplierCostReviewState(matchingRow, reviews[matchingRow.key], draft).requiresUpdate) {
    ...
    const response = await appClient.functions.invoke('variableChargesSupplierVerify', {
      ...
      rowOutcomes: stageRows.map((row) => ({
        ...
        outcome: supplierCostReviewState(row, reviews[row.key], extraDrafts[row.sourceId]).outcome,
  ...
  const confirmPairedSides = async (requirement, sides) => {
    ...
    for (const row of stageRows) {
      ...
      if (costSelected && !supplierCostReviewState(row, review, extraDrafts[row.sourceId]).valid) {
        setSaveError(`Complete the cost details for ${itemLabel(row)}, or mark an unchanged cost Correct.`); return;
      }
    ...
    (detail?.extraCosts || []).forEach((item, index) => {
      ...
      const costReview = row ? supplierCostReviewState(row, reviews[row.key], draft) : null;
      if (costSelected && costReview?.outcome === 'cancelled' && draft.cancelled && !original.cancelled) {
        cancellations.push({ extraCostId: id, expectedLastModifiedDate });
      } else if (costSelected && costReview?.requiresUpdate) {
    ...
    const response = await appClient.functions.invoke('variableChargesSideConfirm', {
      ...
      cost: costSelected ? {
        ...
        rowOutcomes: stageRows.map((row) => ({ sourceId: row.sourceId, outcome: supplierCostReviewState(row, reviews[row.key], extraDrafts[row.sourceId]).outcome, evidenceDocumentIds: reviews[row.key]?.evidenceDocumentIds || [] })),
    ...
    <div className="space-y-5 p-4 lg:p-8">
      ...
      {detailLoading ? <div aria-busy="true" aria-live="polite"><span className="sr-only">Refreshing Variable Charges task</span><VariableChargeReviewSkeleton /></div> : detailError ? (
        ...
        <div className="space-y-5">
          ...
          {pairedWorkflow && activeCase.hongKongVariableCharges === true && detail.supplierDualCurrencySummary && (() => {
              ...
              <div className="space-y-3">
                {activeSupplierRows.map((row) => {
                  ...
                  return <SupplierChargeDecision key={row.key} row={row} review={review} draft={draft} disabled={!canSupplierEdit || Boolean(supplierSavingId)} onOutcome={(outcome) => {
                    ...
                    if (extraId && outcome === 'correct') updateExtraDraft(extraId, { cancelled: false });
...
function itemLabel(row) {
  ...
  return row.item?.hongKongVariableCharges === true && isPortClearanceItem(row.item)
    ? 'Port Clearance Fee / Extension'
    : productName || 'Product name unavailable';
}

function supplierItemLabel(row) {
  return row.item?.hongKongVariableCharges === true && isPortClearanceItem(row.item) ? 'Port Clearance Fee' : itemLabel(row);
}

function buyerItemLabel(row) {
  return row.item?.hongKongVariableCharges === true && isPortClearanceItem(row.item) ? 'Port Clearance Extension' : itemLabel(row);
}
...
function PairedExtraCostFields({ row, draft, disabled, onChange }) {
  ...
  const ui = supplierChargeUiModel(row.item);
  const currencyLocked = ui.hongKongVariableCharges && Boolean(draft.requiredInputCurrency);
  return <div className="mt-3 space-y-3 rounded-lg border border-amber-200 bg-amber-50/40 p-3"><div className="text-xs font-semibold uppercase tracking-wide text-amber-950">Edit Charge Details</div><div className="space-y-2"><RequiredLabel htmlFor={`${id}-description`}>Description</RequiredLabel><Input id={`${id}-description`} aria-required="true" value={draft.description || ''} disabled={disabled} onChange={(event) => onChange({ description: event.target.value })} /></div><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><RequiredLabel>Pricing Basis</RequiredLabel><Select value={draft.pricingType} disabled={disabled} onValueChange={(pricingType) => onChange({ pricingType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="fixed">Fixed</SelectItem><SelectItem value="per_unit">Per Unit</SelectItem></SelectContent></Select></div>{ui.showCurrencySelector && <div className="space-y-2"><RequiredLabel>{currencyLocked ? 'Agent Agreed Currency' : 'Input Currency'}</RequiredLabel><Select value={draft.inputCurrency || 'USD'} disabled={disabled || currencyLocked} onValueChange={(inputCurrency) => onChange({ inputCurrency })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="HKD">HKD</SelectItem><SelectItem value="USD">USD</SelectItem></SelectContent></Select>{currencyLocked && <div className="text-xs text-amber-900">All costs for this agent use the Account’s agreed currency.</div>}</div>}<div className="space-y-2"><RequiredLabel htmlFor={`${id}-cost`}>{draft.pricingType === 'fixed' ? 'Supplier Fixed Cost' : 'Supplier Unit Cost'} ({ui.hongKongVariableCharges ? draft.inputCurrency || 'USD' : 'USD'})</RequiredLabel><Input id={`${id}-cost`} aria-required="true" inputMode="decimal" value={draft.supplierCost ?? ''} disabled={disabled} onChange={(event) => onChange({ supplierCost: event.target.value })} /></div>{draft.pricingType === 'per_unit' && <><div className="space-y-2"><RequiredLabel htmlFor={`${id}-quantity`}>Quantity</RequiredLabel><Input id={`${id}-quantity`} aria-required="true" inputMode="decimal" value={draft.quantity ?? ''} disabled={disabled} onChange={(event) => onChange({ quantity: event.target.value })} /></div><div className="space-y-2"><RequiredLabel htmlFor={`${id}-uom`}>UOM</RequiredLabel><Input id={`${id}-uom`} aria-required="true" value={draft.unitOfMeasure || ''} disabled={disabled} onChange={(event) => onChange({ unitOfMeasure: event.target.value })} /></div></>}</div></div>;
}
...
function SupplierDualAmount({ label, row, draft, total = false, companyRate }) {
  ...
  return <div><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 font-semibold tabular-nums">{usd == null ? 'Unavailable' : formatMoney(usd, 'USD')}</div>{ui.showHkdPreview && <><div className="text-xs font-medium tabular-nums text-slate-600">{hkd == null ? 'HKD unavailable' : formatMoney(hkd, 'HKD')}</div>{serverDual?.basis === 'current_rate' && <div className="mt-0.5 text-[11px] text-amber-700">Current company rate</div>}</>}</div>;
}
...
function PairedChargeRow({ row, review, draft, currency, companyRate, canCostEdit, canBuyerEdit, costApproved: costApprovedProp = false, buyerApproved: buyerApprovedProp = false, instanceUrl, onReviewChange, onDraftChange }) {
  ...
  const supplierLocked = usesHongKongChargeUi(row.item) && supplierCostLockedForItem(row.item);
  const buyerLocked = usesHongKongChargeUi(row.item) && buyerDecisionLockedForItem(row.item);
  const includedInBasicCalling = usesHongKongChargeUi(row.item) && isIncludedBasicCallingItem(row.item);
  ...
  const changePortClearance = (patch) => {
    ...
    onDraftChange({
      ...
      ...(calculation.complete ? {
        pricingType: 'fixed',
        supplierCost: draft?.inputCurrency === 'USD' ? calculation.supplierTotalUsd : calculation.supplierHkd,
...
function PairedNewChargeRow({ draft, products, canCostEdit, canBuyerEdit, commonOwner, onChange, onRemove }) {
  ...
  const currencyLocked = ui.hongKongVariableCharges && Boolean(draft.requiredInputCurrency);
  return <article className="border-t border-dashed border-blue-300"><div className="flex items-center justify-between border-b border-border bg-blue-50/40 px-4 py-3"><div><div className="font-semibold">New Extra Cost</div><div className="text-xs text-muted-foreground">Supplier and payment term are inherited.</div></div><Button type="button" size="sm" variant="ghost" onClick={onRemove} disabled={!canCostEdit}><X className="mr-1 h-4 w-4" />Remove</Button></div><div className="grid grid-cols-2"><section className="space-y-3 border-r-2 border-slate-300 bg-slate-50/30 p-4"><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><RequiredLabel>Product</RequiredLabel><Select value={draft.productId || undefined} disabled={!canCostEdit} onValueChange={(productId) => onChange({ productId })}><SelectTrigger><SelectValue placeholder="Select Product" /></SelectTrigger><SelectContent>{products.map((product) => <SelectItem key={valueOf(product, ['id', 'Id'])} value={String(valueOf(product, ['id', 'Id']))}>{valueOf(product, ['name', 'Name'], 'Product')}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><RequiredLabel>Description</RequiredLabel><Input value={draft.description || ''} disabled={!canCostEdit} onChange={(event) => onChange({ description: event.target.value })} /></div><div className="space-y-2"><RequiredLabel>Pricing Basis</RequiredLabel><Select value={pricingType} disabled={!canCostEdit} onValueChange={(value) => onChange({ pricingType: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="fixed">Fixed</SelectItem><SelectItem value="per_unit">Per Unit</SelectItem></SelectContent></Select></div>{ui.showCurrencySelector && <div className="space-y-2"><RequiredLabel>{currencyLocked ? 'Agent Agreed Currency' : 'Input Currency'}</RequiredLabel><Select value={draft.inputCurrency || 'USD'} disabled={!canCostEdit || currencyLocked} onValueChange={(inputCurrency) => onChange({ inputCurrency })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="HKD">HKD</SelectItem><SelectItem value="USD">USD</SelectItem></SelectContent></Select>{currencyLocked && <div className="text-xs text-amber-900">All costs for this agent use the Account’s agreed currency.</div>}</div>}<div className="space-y-2"><RequiredLabel>{pricingType === 'fixed' ? 'Supplier Fixed Cost' : 'Supplier Unit Cost'} ({ui.hongKongVariableCharges ? draft.inputCurrency || 'USD' : 'USD'})</RequiredLabel><Input inputMode="decimal" value={draft.supplierCost ?? ''} disabled={!canCostEdit} onChange={(event) => onChange({ supplierCost: event.target.value })} /></div>{pricingType === 'per_unit' && <><div className="space-y-2"><RequiredLabel>Quantity</RequiredLabel><Input inputMode="decimal" value={draft.quantity ?? ''} disabled={!canCostEdit} onChange={(event) => onChange({ quantity: event.target.value })} /></div><div className="space-y-2"><RequiredLabel>UOM</RequiredLabel><Input value={draft.unitOfMeasure || ''} disabled={!canCostEdit} onChange={(event) => onChange({ unitOfMeasure: event.target.value })} /></div></>}</div></section><section className="space-y-3 bg-blue-50/20 p-4">{commonOwner ? <><DecisionButtons ariaLabel="New charge buyer review" selected={draft.buyerChargeDecision || ''} disabled={!canBuyerEdit} onChange={(buyerChargeDecision) => onChange({ buyerChargeDecision })} options={[{ value: '', label: 'Pending', tone: 'bg-slate-100 text-slate-800' }, { value: 'include', label: 'Charge Buyer', tone: 'bg-blue-100 text-blue-900' }, { value: 'exclude', label: 'Do Not Charge', tone: 'bg-slate-200 text-slate-900' }]} />{draft.buyerChargeDecision === 'include' && <div className="space-y-2"><RequiredLabel>Buyer {pricingType === 'fixed' ? 'Fixed Charge' : 'Unit Price'} (USD)</RequiredLabel><Input inputMode="decimal" value={draft.buyerPrice ?? ''} disabled={!canBuyerEdit} onChange={(event) => onChange({ buyerPrice: event.target.value })} /></div>}<div><div className="text-xs text-muted-foreground">Margin</div><MarginAmount value={draft.buyerChargeDecision === 'exclude' && supplierTotal != null ? -supplierTotal : margin} currency="USD" /></div></> : <div className="rounded-md border border-blue-200 bg-white/70 p-3 text-sm text-blue-950">Pending for Buyer Trader after Supplier approval.</div>}</section></div></article>;
}
...
function NewExtraCostEditor({ draft, products, supplierAccounts, files, defaultPaymentTerm, supplierStage = false, disabled, onChange, onRemove }) {
  ...
  return <div className="rounded-lg border border-dashed border-primary/40 bg-primary/[0.02] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="font-medium">New supplier charge</div><div className="mt-1 text-xs text-muted-foreground">The payment term is inherited automatically.</div></div><Button type="button" variant="ghost" size="sm" onClick={onRemove} disabled={disabled}><X className="mr-1 h-4 w-4" /> Remove</Button></div><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="space-y-2"><RequiredLabel>Supplier</RequiredLabel><Select value={draft.supplierAccountId || undefined} onValueChange={(supplierAccountId) => onChange({ supplierAccountId })} disabled={disabled || supplierStage}><SelectTrigger><SelectValue placeholder="Select supplier" /></SelectTrigger><SelectContent>{supplierAccounts.map((account) => <SelectItem key={account.id} value={String(account.id)}>{account.name || account.id}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><RequiredLabel>Product</RequiredLabel><Select value={draft.productId || undefined} onValueChange={(productId) => onChange({ productId })} disabled={disabled}><SelectTrigger><SelectValue placeholder="Select product" /></SelectTrigger><SelectContent>{products.map((product) => <SelectItem key={valueOf(product, ['id', 'Id', 'productId'])} value={String(valueOf(product, ['id', 'Id', 'productId']))}>{valueOf(product, ['name', 'Name', 'label'], 'Product')}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><RequiredLabel htmlFor={`new-extra-description-${draft.localId}`}>Description</RequiredLabel><Input id={`new-extra-description-${draft.localId}`} value={draft.description} onChange={(event) => onChange({ description: event.target.value })} disabled={disabled} /></div></div><ExtraCostFields id={`new-${draft.localId}`} value={{ ...draft, paymentTerm: draft.paymentTerm || defaultPaymentTerm }} disabled={disabled} supplierStage={supplierStage} onChange={onChange} paymentTermReadOnly />{!supplierStage && <div className="mt-4 grid gap-3 md:grid-cols-3"><label className="flex items-center gap-2 text-sm font-medium"><Checkbox checked={draft.reviewed === true} disabled={disabled} onCheckedChange={(checked) => onChange({ reviewed: checked === true })} /> Reviewed</label><div className="space-y-2"><RequiredLabel>Buyer-charge decision</RequiredLabel><Select value={draft.buyerChargeDecision || undefined} onValueChange={(buyerChargeDecision) => onChange({ buyerChargeDecision })} disabled={disabled}><SelectTrigger><SelectValue placeholder="Select decision" /></SelectTrigger><SelectContent>{BUYER_CHARGE_DECISIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><RequiredLabel htmlFor={`new-extra-reference-${draft.localId}`}>Reference or review note</RequiredLabel><Input id={`new-extra-reference-${draft.localId}`} value={draft.referenceOrNote || ''} onChange={(event) => onChange({ referenceOrNote: event.target.value })} disabled={disabled} /></div></div>}{files.length ? <div className="mt-3 flex flex-wrap gap-2">{files.map((file) => { const fileId = String(valueOf(file, ['contentDocumentId', 'id', 'Id'])); return <label key={fileId} className="flex items-center gap-2 rounded-md border border-border px-2 py-1.5 text-xs"><Checkbox checked={(draft.evidenceDocumentIds || []).includes(fileId)} disabled={disabled} onCheckedChange={() => toggleFile(fileId)} /><FileText className="h-3.5 w-3.5" />{valueOf(file, ['title', 'name'], 'Salesforce File')}</label>; })}</div> : null}</div>;
}

function ExtraCostFields({ id, value, disabled, supplierStage = false, buyerStage = false, onChange, paymentTermReadOnly = false }) {
  const ui = supplierChargeUiModel(value);
  return <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-6"><div className="space-y-2"><Label htmlFor={`${id}-term`}>Payment term</Label><Input id={`${id}-term`} value={value.paymentTerm || ''} onChange={(event) => onChange({ paymentTerm: event.target.value })} disabled={disabled || paymentTermReadOnly || buyerStage} /></div><div className="space-y-2"><RequiredLabel>Pricing</RequiredLabel><Select value={value.pricingType} onValueChange={(pricingType) => onChange({ pricingType })} disabled={disabled || buyerStage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="fixed">Fixed</SelectItem><SelectItem value="per_unit">Per unit</SelectItem></SelectContent></Select></div>{supplierStage && ui.showCurrencySelector && <div className="space-y-2"><RequiredLabel>Input Currency</RequiredLabel><Select value={value.inputCurrency || 'USD'} onValueChange={(inputCurrency) => onChange({ inputCurrency })} disabled={disabled || buyerStage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="HKD">HKD</SelectItem><SelectItem value="USD">USD</SelectItem></SelectContent></Select></div>}<div className="space-y-2"><RequiredLabel htmlFor={`${id}-supplier-cost`}>{value.pricingType === 'fixed' ? 'Fixed supplier cost' : 'Supplier cost / unit'} ({ui.hongKongVariableCharges ? value.inputCurrency || 'USD' : 'USD'})</RequiredLabel><Input id={`${id}-supplier-cost`} inputMode="decimal" value={value.supplierCost ?? ''} onChange={(event) => onChange({ supplierCost: event.target.value })} disabled={disabled || buyerStage} /></div>{!supplierStage && <div className="space-y-2"><RequiredLabel htmlFor={`${id}-buyer-price`}>{value.pricingType === 'fixed' ? 'Fixed buyer price' : 'Buyer price / unit'} (USD)</RequiredLabel><Input id={`${id}-buyer-price`} inputMode="decimal" value={value.buyerPrice ?? ''} onChange={(event) => onChange({ buyerPrice: event.target.value })} disabled={disabled} /></div>}{value.pricingType === 'per_unit' && <><div className="space-y-2"><RequiredLabel htmlFor={`${id}-quantity`}>Quantity</RequiredLabel><Input id={`${id}-quantity`} aria-required="true" inputMode="decimal" value={value.quantity ?? ''} onChange={(event) => onChange({ quantity: event.target.value })} disabled={disabled || buyerStage} /></div><div className="space-y-2"><RequiredLabel htmlFor={`${id}-uom`}>Unit of measure</RequiredLabel><Input id={`${id}-uom`} aria-required="true" value={value.unitOfMeasure || ''} onChange={(event) => onChange({ unitOfMeasure: event.target.value })} disabled={disabled || buyerStage} /></div></>}</div>;
}

function PostInvoiceResolution({ value, disabled, saving, onChange, onSave }) { return <section className="space-y-3 rounded-lg border border-rose-300 bg-rose-50 p-4"><h2 className="text-sm font-semibold text-rose-950">Invoice already issued—action required</h2><div className="space-y-2"><RequiredLabel>Resolution</RequiredLabel><Select value={value.resolution} onValueChange={(resolution) => onChange({ resolution })} disabled={disabled || saving}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{POST_INVOICE_RESOLUTIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="post-invoice-note">Resolution Note (Optional)</Label><Textarea id="post-invoice-note" value={value.note} onChange={(event) => onChange({ note: event.target.value })} disabled={disabled || saving} /></div><Button type="button" variant="outline" onClick={onSave} disabled={disabled || saving}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save Resolution</Button></section>; }

export const variableChargeUiInternals = Object.freeze({
  initialAddDraft,
  initialExtraDraft,
  supplierCostReviewState,
  isHongKongPortClearanceItem,
  itemLabel,
  PairedExtraCostFields,
  supplierChargeUiModel,
  variableChargeUiQuantityLabel,
  SupplierDualAmount,
});
