...
function LegHeader({ leg, name, nameControl = null, traderLabel, side, paymentTerm, buyer = false, editable = false, assignmentSaving, canGmOverride = false, onReviewAsGm, onAssign }) {
    ...
    <section className="border-t-2 border-amber-300 bg-amber-50/35 p-4">
      ...
      {calculation.complete ? (
        <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm">
          ...
          <span>Chargeable <strong>{calculation.locations.map((row) => `${row.chargeableHours}h ${row.location === ANCHORAGE_LOCATION_ELSEWHERE ? ANCHORAGE_LOCATION_ELSEWHERE_LABEL : row.location}`).join(' · ') || '0 hours'}</strong></span>
