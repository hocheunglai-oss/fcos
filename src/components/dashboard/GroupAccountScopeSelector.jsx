...
export default function GroupAccountScopeSelector({ groupScope, selectedAccountIds = null, onChange, disabled = false }) {
  ...
  return <section className="rounded-lg border border-sky-200 bg-sky-50/70 p-3" aria-label="GROUP Account scope">
    ...
    <details className="mt-3 rounded-md border border-sky-200 bg-white/60 p-2" data-testid="group-account-selection">
      <summary className="cursor-pointer text-xs font-semibold text-sky-950">Choose Accounts · {selected.size} of {available.length} selected</summary>
      ...
      {visible.map((account) => {
        ...
        const duplicateName = available.filter((candidate) => candidate.name === account.name).length > 1;
        return <button
        ...
        >
          ...
          <span className="break-words">{account.isGroupRoot ? `GROUP account · ${account.name}` : account.name}{duplicateName && account.clKey ? ` · ${account.clKey}` : ''}</span>
        </button>;
      })}
      </div>
    </details>
