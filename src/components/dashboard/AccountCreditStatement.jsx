...
export default function AccountCreditStatement({ accountId, entityType = 'account', includedGroupAccountIds = null, onGroupScopeChange, active, statementSide = 'buyer', availableStatementSides = ['buyer'], onStatementSideChange, onStemClick }) {
  ...
  const load = useCallback(async ({ cursor = null, history = [], force = false } = {}) => {
    ...
    try {
      const response = await appClient.functions.invoke('dashboardAccountCreditStatement', {
        accountId, entityId: accountId, entityType, includedAccountIds: includedGroupAccountIds, scope, cursor, limit: 50, force,
  ...
  }, [accountId, active, entityType, includedGroupAccountIds, scope]);
  ...
  return <div className="space-y-5" data-testid="account-credit-statement">
    ...
    <section className="space-y-3">
      ...
      <div className={`grid gap-3 ${entityType === 'group' ? '' : 'xl:grid-cols-2'}`}>
        {entityType !== 'group' ? <CreditPositionPanel title="Selected Account" reconciliation={result.reconciliation.individual} currency={currency} cards={accountPositionCards(result.credit, result.reconciliation.individual)} /> : null}
        {result.group ? <CreditPositionPanel title={entityType === 'group' ? 'Selected GROUP Accounts' : 'GROUP'} reconciliation={result.reconciliation.group} currency={currency} cards={groupPositionCards(result.credit, result.reconciliation.group)} /> : <CreditPositionPanel title="GROUP" reconciliation={{ notApplicable: true }} currency={currency} emptyMessage="No Salesforce GROUP ancestor applies to this Account." />}
      </div>
