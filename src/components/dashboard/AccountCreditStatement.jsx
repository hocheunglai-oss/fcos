...
export default function AccountCreditStatement({ accountId, active, onStemClick }) {
  ...
  return <div className="space-y-5" data-testid="account-credit-statement">
    ...
    {result.creditResolution?.mode === 'same_name_fallback' ? <div className="rounded-lg border border-sky-200 bg-sky-50 p-3 text-sm text-sky-950"><div className="font-semibold">Salesforce credit snapshot resolved</div><p className="mt-1 text-xs">{result.creditResolution.notice}</p><p className="mt-1 text-xs font-medium">Fallback CL Key: {result.creditResolution.clKey || 'Unavailable'} · Reconciliation window: {displayDate(result.creditResolution.reconciliationWindowStart)}</p></div> : null}
    {projectionWarnings.length ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3"><div className="flex items-center gap-2 text-sm font-semibold text-amber-950"><AlertTriangle className="h-4 w-4" />Projection safeguards</div><ul className="mt-2 space-y-1 text-xs text-amber-900">{projectionWarnings.map((warning) => <li key={warning}>• {warning}</li>)}</ul></div> : null}
    <div className="flex flex-wrap gap-2"><ReconciliationBadge label="Individual" result={result.reconciliation.individual} />{result.group ? <ReconciliationBadge label="GROUP" result={result.reconciliation.group} /> : null}<span className="rounded-full bg-sky-100 px-2 py-1 text-xs font-semibold text-sky-900">Category: {result.credit.category || 'Unavailable'}</span></div>
