...
function UsedCreditBridge({ bridge, fallbackCurrency }) {
  if (!bridge) return null;
  const currency = bridge.currency || fallbackCurrency;
  const status = {
    buyer_only_reconciled: { label: 'Reconciled — buyer only', className: 'bg-emerald-100 text-emerald-900' },
    supplier_payables_explained: { label: 'Explained — supplier Cashflow exposure', className: 'bg-sky-100 text-sky-900' },
    incomplete: { label: 'Incomplete evidence', className: 'bg-amber-100 text-amber-900' },
    unexplained: { label: 'Unexplained variance', className: 'bg-amber-100 text-amber-900' },
  }[bridge.status] || { label: 'Reconciliation unavailable', className: 'bg-muted text-muted-foreground' };
  const evidence = bridge.evidence || [];
  const unexplainedResidual = numeric(bridge.unexplainedResidual);
  const residualReconciled = unexplainedResidual != null && Math.abs(unexplainedResidual) <= (bridge.tolerance || 1);
  return <section className="rounded-xl border border-border bg-card p-4" data-testid="used-credit-bridge">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-semibold">Salesforce used-credit reconciliation</h3><p className="mt-1 text-xs text-muted-foreground">Salesforce used credit remains authoritative. FCOS explains its population without treating supplier-side exposure as buyer credit risk.</p></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${status.className}`}>{status.label}</span></div>
    <div className="mt-4 grid gap-2 text-sm sm:grid-cols-2 xl:grid-cols-5">
      <div className="rounded-lg bg-muted/40 p-3"><div className="text-[11px] font-semibold uppercase text-muted-foreground">Buyer receivables</div><div className="mt-1 font-semibold tabular-nums">{money(bridge.buyerReceivables, currency)}</div><div className="mt-1 text-[11px] text-muted-foreground">{bridge.buyerStemCount ?? 0} buyer STEMs</div></div>
      <div className="rounded-lg bg-muted/40 p-3"><div className="text-[11px] font-semibold uppercase text-muted-foreground">Supplier exposure included</div><div className="mt-1 font-semibold tabular-nums">+ {money(bridge.supplierPayables, currency)}</div><div className="mt-1 text-[11px] text-muted-foreground">{bridge.supplierStemCount ?? 0} supplier STEMs</div></div>
      <div className="rounded-lg bg-muted/40 p-3"><div className="text-[11px] font-semibold uppercase text-muted-foreground">Reconstructed used</div><div className="mt-1 font-semibold tabular-nums">= {money(bridge.reconstructedUsed, currency)}</div></div>
      <div className="rounded-lg bg-muted/40 p-3"><div className="text-[11px] font-semibold uppercase text-muted-foreground">Salesforce used</div><div className="mt-1 font-semibold tabular-nums">{money(bridge.salesforceUsed, currency)}</div><div className="mt-1 text-[11px] text-muted-foreground">Authoritative snapshot</div></div>
      <div className={`rounded-lg p-3 ${residualReconciled ? 'bg-emerald-50 text-emerald-950' : 'bg-amber-50 text-amber-950'}`}><div className="text-[11px] font-semibold uppercase">Unexplained residual</div><div className="mt-1 font-semibold tabular-nums">{money(bridge.unexplainedResidual, currency)}</div><div className="mt-1 text-[11px]">Tolerance {money(bridge.tolerance, currency)}</div></div>
    </div>
    {evidence.length ? <details className="mt-4 rounded-lg border border-border"><summary className="cursor-pointer px-3 py-2 text-xs font-semibold">Supplier Cashflow evidence ({evidence.length})</summary><div className="overflow-x-auto border-t border-border"><table className="w-full min-w-[900px] text-xs"><thead className="bg-muted/40 text-left uppercase text-muted-foreground"><tr><th className="px-3 py-2">Supplier / STEM</th><th className="px-3 py-2">Effective</th><th className="px-3 py-2">Due</th><th className="px-3 py-2 text-right">Included gross</th><th className="px-3 py-2 text-right">Current balance</th><th className="px-3 py-2 text-right">Received / paid</th></tr></thead><tbody>{evidence.map((row) => <tr key={`${row.accountId}:${row.stemId}:${row.currency}`} className="border-t border-border"><td className="px-3 py-2"><div className="font-medium">{row.accountName || row.accountId}</div><div className="text-muted-foreground">{row.stemName || row.stemId}</div></td><td className="px-3 py-2">{displayDate(row.effectiveDate)}</td><td className="px-3 py-2">{displayDate(row.dueDate)}</td><td className="px-3 py-2 text-right tabular-nums">{money(row.grossAmount, row.currency || currency)}</td><td className="px-3 py-2 text-right tabular-nums">{money(row.currentBalance, row.currency || currency)}</td><td className="px-3 py-2 text-right tabular-nums">{money(row.receivedPaidAmount, row.currency || currency)}</td></tr>)}</tbody></table></div></details> : null}
  </section>;
}
...
export default function AccountCreditStatement({ accountId, entityType = 'account', includedGroupAccountIds = null, onGroupScopeChange, onForecastConservativenessChange, hideGroupScopeSelector = false, active, statementSide = 'buyer', availableStatementSides = ['buyer'], onStatementSideChange, onStemClick }) {
  ...
  return <div className="space-y-5" data-testid="account-credit-statement">
    ...
    {!hideGroupScopeSelector ? <GroupAccountScopeSelector groupScope={result.groupScope} selectedAccountIds={includedGroupAccountIds} onChange={(ids) => { setNavigation({ cursor: null, history: [] }); setSelectedInvoiceIds(new Set()); onGroupScopeChange?.(ids); }} disabled={loading} /> : null}
    ...
    <section className="space-y-3">
      ...
      <UsedCreditBridge bridge={result.usedCreditBridge} fallbackCurrency={currency} />
