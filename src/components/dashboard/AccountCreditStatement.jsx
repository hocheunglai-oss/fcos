...
export default function AccountCreditStatement({ accountId, active, onStemClick }) {
  ...
  const copySelectedInvoices = async () => {
    const totalLines = Object.entries(selectedInvoiceTotals).map(([totalCurrency, amount]) => `Total invoice amount - ${reminderCopyMoney(amount, totalCurrency)}`);
    const copyText = paymentReminderCopyText(selectedInvoiceRows.map((row) => ({
      ...
      amount: reminderCopyMoney(row.buyerInvoiceAmount, row.currency || currency),
      dueDate: displayDate(row.buyerInvoiceDueDate),
      status: reminderCopyStatus(row.buyerInvoiceDaysUntilDue),
    })), totalLines);
    try {
    ...
    <section className="rounded-xl border border-border bg-card">
      <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="mr-1 text-xs text-muted-foreground">{selectedInvoiceRows.length ? <><span className="font-semibold text-foreground">{selectedInvoiceRows.length} selected</span>{Object.entries(selectedInvoiceTotals).map(([totalCurrency, amount]) => <span key={totalCurrency}> · Total invoice amount {money(amount, totalCurrency)}</span>)}</> : `${selectableInvoiceRows.length} invoiced STEM${selectableInvoiceRows.length === 1 ? '' : 's'} available`}</div>
          <Button type="button" size="sm" variant="outline" onClick={toggleAllInvoices} disabled={!selectableInvoiceRows.length}>{selectedInvoiceRows.length === selectableInvoiceRows.length && selectableInvoiceRows.length ? 'Clear selection' : 'Select all invoiced'}</Button>
