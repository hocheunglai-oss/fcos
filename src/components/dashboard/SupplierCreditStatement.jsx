...
function PayableChart({ chart, includeGroup, statementSide, availableStatementSides, onStatementSideChange }) {
  ...
  return <div className="rounded-xl border border-border bg-card p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-semibold">{chart.currency} remaining payable forecast</h4><p className="mt-1 text-xs text-muted-foreground">Square steps descend only on evidenced payment dates. Undated obligations remain in the final plateau.</p></div><div className="flex flex-col items-end gap-2"><CreditStatementSideToggle value={statementSide} availableSides={availableStatementSides} onChange={onStatementSideChange} /><div className="flex flex-wrap justify-end gap-3 text-xs"><span className="flex items-center gap-1.5"><span className="h-0.5 w-5 bg-sky-600" />Account · {money(chart.account?.opening, chart.currency)}</span>{includeGroup ? <span className="flex items-center gap-1.5"><span className="h-0.5 w-5 bg-violet-600" />GROUP · {money(chart.group?.opening, chart.currency)}</span> : null}</div></div></div><div className="mt-4 h-72 w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 8, right: 12, left: 12, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} minTickGap={28} /><YAxis tickFormatter={(value) => Math.abs(value) >= 1_000_000 ? `${(value / 1_000_000).toFixed(1)}M` : Math.abs(value) >= 1_000 ? `${Math.round(value / 1_000)}k` : value} width={56} /><Tooltip labelFormatter={displayDate} formatter={(value, name) => [money(value, chart.currency), name === 'group' ? 'GROUP remaining' : 'Account remaining']} /><Line type="stepAfter" dataKey="account" name="account" stroke="#0284c7" strokeWidth={3} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} />{includeGroup ? <Line type="stepAfter" dataKey="group" name="group" stroke="#7c3aed" strokeWidth={3} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} /> : null}</LineChart></ResponsiveContainer></div>{chart.account?.undatedExposure > 0 || chart.group?.undatedExposure > 0 ? <div className="mt-2 text-xs text-amber-800">Undated residual: Account {money(chart.account?.undatedExposure, chart.currency)}{includeGroup ? ` · GROUP ${money(chart.group?.undatedExposure, chart.currency)}` : ''}</div> : null}</div>;
}
...
export default function SupplierCreditStatement({ accountId, active, filters = {}, statementSide = 'supplier', availableStatementSides = ['supplier'], onStatementSideChange, onStemClick }) {
