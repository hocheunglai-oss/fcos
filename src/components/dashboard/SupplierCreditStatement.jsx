...
function PayableChart({ chart, includeGroup, groupOnly = false, statementSide, availableStatementSides, onStatementSideChange }) {
  ...
  return <div className="rounded-xl border border-border bg-card p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-semibold">{chart.currency} remaining payable forecast</h4><p className="mt-1 text-xs text-muted-foreground">Square steps descend only on evidenced payment dates. Undated obligations remain in the final plateau.</p></div><div className="flex flex-col items-end gap-2"><CreditStatementSideToggle value={statementSide} availableSides={availableStatementSides} onChange={onStatementSideChange} /><div className="flex flex-wrap justify-end gap-3 text-xs">{!groupOnly ? <span className="flex items-center gap-1.5"><span className="h-0.5 w-5 bg-sky-600" />Account · {money(chart.account?.opening, chart.currency)}</span> : null}{includeGroup ? <span className="flex items-center gap-1.5"><span className="h-0.5 w-5 bg-violet-600" />{groupOnly ? 'Selected GROUP Accounts' : 'GROUP'} · {money(chart.group?.opening, chart.currency)}</span> : null}</div></div></div><div className="mt-4 h-72 w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 8, right: 12, left: 12, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} minTickGap={28} /><YAxis tickFormatter={(value) => Math.abs(value) >= 1_000_000 ? `${(value / 1_000_000).toFixed(1)}M` : Math.abs(value) >= 1_000 ? `${Math.round(value / 1_000)}k` : value} width={56} /><Tooltip labelFormatter={displayDate} formatter={(value, name) => [money(value, chart.currency), name === 'group' ? groupOnly ? 'Selected GROUP Accounts remaining' : 'GROUP remaining' : 'Account remaining']} />{!groupOnly ? <Line type="stepAfter" dataKey="account" name="account" stroke="#0284c7" strokeWidth={3} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} /> : null}{includeGroup ? <Line type="stepAfter" dataKey="group" name="group" stroke="#7c3aed" strokeWidth={3} dot={{ r: 3 }} connectNulls={false} isAnimationActive={false} /> : null}</LineChart></ResponsiveContainer></div>{(!groupOnly && chart.account?.undatedExposure > 0) || chart.group?.undatedExposure > 0 ? <div className="mt-2 text-xs text-amber-800">Undated residual: {!groupOnly ? `Account ${money(chart.account?.undatedExposure, chart.currency)}` : ''}{includeGroup ? `${!groupOnly ? ' · ' : ''}${groupOnly ? 'Selected GROUP Accounts' : 'GROUP'} ${money(chart.group?.undatedExposure, chart.currency)}` : ''}</div> : null}</div>;
}

function evidenceSummary(row) {
  if (row.rowType === 'issued') return `Supplier Invoice ${row.supplierInvoiceName} · Invoice ${money(row.invoiceAmount, row.currency)} · Payable ${money(row.payableBalance, row.currency)}${row.partialDueDate ? ` · Partial ${money(row.partialAmount, row.currency)} due ${displayDate(row.partialDueDate)}` : ''}`;
...
export default function SupplierCreditStatement({ accountId, entityType = 'account', includedGroupAccountIds = null, onGroupScopeChange, active, filters = {}, statementSide = 'supplier', availableStatementSides = ['supplier'], onStatementSideChange, onStemClick }) {
  ...
  const groupIncluded = entityType === 'group' || includeGroup;
  ...
  const load = useCallback(async ({ cursor = null, history = [], force = false } = {}) => {
    ...
    try {
      const response = await appClient.functions.invoke('dashboardAccountCreditStatement', {
        accountId, entityId: accountId, entityType, includedAccountIds: includedGroupAccountIds, side: 'supplier', scope, includeGroup: groupIncluded, cursor, limit: 50, filters: filterPayload, force,
  ...
  return <div className="space-y-5" data-testid="supplier-credit-statement">
    ...
    <section className="space-y-4">{result.chart?.currencies?.map((chart) => <PayableChart key={chart.currency} chart={chart} includeGroup={groupIncluded} groupOnly={entityType === 'group'} statementSide={statementSide} availableStatementSides={availableStatementSides} onStatementSideChange={onStatementSideChange} />)}{!result.chart?.currencies?.length ? <div className="rounded-xl border border-dashed border-border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><span className="text-sm text-muted-foreground">No payable forecast exists for this scope.</span><CreditStatementSideToggle value={statementSide} availableSides={availableStatementSides} onChange={onStatementSideChange} /></div></div> : null}</section>
