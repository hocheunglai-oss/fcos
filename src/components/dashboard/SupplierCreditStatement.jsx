...
function StatementCard({ row, selected, onSelect, onStemClick }) {
  const selectable = row.exposureComplete && numeric(row.currentExposure) != null;
...
export default function SupplierCreditStatement({ accountId, active, filters = {}, onStemClick }) {
  ...
  const selectableRows = rows.filter((row) => row.exposureComplete && numeric(row.currentExposure) != null);
