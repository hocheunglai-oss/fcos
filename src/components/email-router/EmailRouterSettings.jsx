...
import { AlertTriangle, ArrowDown, ArrowUp, CheckCircle2, Loader2, MailSearch, Pencil, Plus, RefreshCw, Save, Trash2, Users } from 'lucide-react';
...
function blankEditor(type) {
  if (type === 'destination') return { type, id: null, displayName: '', emailAddress: '', active: true, sortOrder: 0, expectedRevision: null };
...
export default function EmailRouterSettings() {
  ...
  const editDestination = (item = null) => setEditor(item ? { type: 'destination', id: item.id, displayName: item.displayName, emailAddress: item.emailAddress, active: item.active, sortOrder: item.sortOrder, expectedRevision: item.revision } : blankEditor('destination'));
  ...
  return <section className="rounded-lg border border-border bg-card p-5">
    ...
    {loading && !configuration ? <StateBlock icon={Loader2} title="Loading Email Router configuration" description="Reading the protected Graph connection and native routing directory." /> : configuration ? <div className="mt-5 space-y-7">
      ...
      <section><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Routing directory</h3><p className="mt-1 text-xs text-muted-foreground">Active FCOS users synchronize automatically. External operational contacts remain configurable here.</p></div><Button size="sm" onClick={() => editDestination()}><Plus />Add contact</Button></div><div className="mt-3 overflow-x-auto border-y border-border"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-muted/40 text-muted-foreground"><tr><th className="px-3 py-2 font-semibold">Name</th><th className="px-3 py-2 font-semibold">Email</th><th className="px-3 py-2 font-semibold">Source</th><th className="px-3 py-2 text-right font-semibold">Action</th></tr></thead><tbody className="divide-y divide-border">{configuration.destinations?.map((item) => <tr key={item.id}><td className="px-3 py-3 font-medium">{item.displayName}</td><td className="px-3 py-3">{item.emailAddress}</td><td className="px-3 py-3 text-muted-foreground">{item.kind === 'fcos_profile' ? 'Active FCOS user' : item.active ? 'External contact' : 'Inactive external contact'}</td><td className="px-3 py-3 text-right">{item.kind === 'provider_directory' ? <Button variant="ghost" size="icon" onClick={() => editDestination(item)} title="Edit contact" aria-label={`Edit ${item.displayName}`}><Pencil /></Button> : <CheckCircle2 className="ml-auto h-4 w-4 text-emerald-600" />}</td></tr>)}</tbody></table></div></section>
    ...
    <Dialog open={Boolean(editor)} onOpenChange={(open) => !open && !busy && setEditor(null)}><DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{editor?.id ? 'Edit' : 'Add'} {editor?.type === 'destination' ? 'external contact' : editor?.type === 'group' ? 'group' : 'routing preset'}</DialogTitle><DialogDescription>Changes apply to future routing selections and are revision protected.</DialogDescription></DialogHeader>{editor && <div className="space-y-4">
