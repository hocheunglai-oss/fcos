...
import { Bell, BellRing, CheckCheck, Loader2, TriangleAlert } from "lucide-react";
...
function sourceBadgeClass(source) {
  return source === "growth_coaching" || source === "growth" || source === "coaching"
    ? "bg-emerald-50 text-emerald-800 ring-emerald-700/10"
    : "bg-blue-50 text-blue-800 ring-blue-700/10";
}
...
export default function WorkNotifications() {
  ...
  const applyResponse = useCallback((data) => {
    ...
    setUnavailableSources(
      Array.isArray(data?.unavailableSources) ? data.unavailableSources : [],
    );
  }, []);
    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          aria-label={unreadCount ? `${unreadCount} unread work notifications` : "Work notifications"}
          title={hasUnavailableSources ? "Some work notifications are temporarily unavailable" : "Work notifications"}
        >
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="mb-1 flex flex-wrap items-center gap-2">
                        <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-medium ring-1 ring-inset", sourceBadgeClass(notification.source))}>
