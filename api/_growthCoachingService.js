...
import {
  growthCalendarCancel,
  growthCalendarConfigured,
  growthCalendarCreate,
  growthCalendarEventPayload,
  growthCalendarGet,
  growthCalendarUpdate,
} from "./_growthOutlook.js";
...
async function reportingAssignments(client) {
  const { data, error } = await client
    .from("growth_reporting_assignments")
    .select("*");
  if (error) throw rpcError(error);
  return data || [];
}
...
function visibleEmployeeIds(assignments, viewerId) {
  const primaryByEmployee = new Map(
    assignments
      .filter((row) => row.primary_manager_id)
      .map((row) => [row.employee_id, row.primary_manager_id]),
  );
...
function normalizeCheckpoints(rows = []) {
  return rows.map((row) => ({
    ...
    evidence: text(row.evidence, 10_000),
    state: row.state
      ? ({ "On track": "On Track", "At risk": "At Risk", "Off track": "Off Track" }[row.state] || row.state)
      : null,
  }));
}
...
async function eventRow(client, {
  subjectType,
  subjectId,
  eventType,
  actor,
...
async function sendNotificationEmail(client, {
  user,
  eventId,
  category,
  dedupeKey,
  ...
  if (
    ...
    || !isExternalActionEnabled("email_delivery")
    || !isExternalActionEnabled("growth_coaching_email")
  ) return;
  ...
  const details = [
    "FCOS Growth & Coaching",
    safeTitle,
    dueDate ? `Date: ${dueDate}` : "",
    progress != null ? `Progress: ${progress}%` : "",
    url ? `Open FCOS: ${url}${PUBLIC_PATH}` : "",
  ].filter(Boolean);
  const html = details
  ...
  try {
    ...
    await client
      .from("growth_email_deliveries")
      .update({ status: "sent", attempt_count: 1, sent_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      .eq("id", reserved.id);
  } catch (error) {
    await client
      .from("growth_email_deliveries")
...
async function sendRoutineDigests(client, today) {
  ...
  const { data: notifications, error } = await client
    ...
    .in("notification_type", ["goal_due", "coaching_action_due"])
  ...
  for (const user of users || []) {
    ...
    const url = publicUrl();
    const lines = [
      "FCOS Growth & Coaching reminder digest",
      `Date: ${today}`,
      ...items.map((row) => `- ${text(row.message || row.title, 255)}`),
      url ? `Open FCOS: ${url}${PUBLIC_PATH}` : "",
    ].filter(Boolean);
    try {
      const from = smtpAuthenticatedFromAddress(
        {},
        `${text(process.env.FCOS_GROWTH_SENDER_NAME || "FCOS", 100)} <${process.env.SMTP_USER || ""}>`,
      );
      await sendWithSmtp({
        from,
        to: user.email,
        ...
        text: lines.join("\n"),
        html: lines.map((line, index) => index === 0
          ? `<h2>${line}</h2>`
          : `<p>${line.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")}</p>`).join(""),
      });
      await client.from("growth_email_deliveries").update({
        status: "sent",
        attempt_count: 1,
...
export async function growthCoachingBootstrap(body = {}, accessContext) {
  const { client, profile } = accessContext;
  const [usersRaw, assignments, relationshipsResult, preferencesResult] = await Promise.all([
    activeUsers(client),
  ...
  const [versionsResult, checkpointsResult, updatesResult, decisionsResult] = goalIds.length
    ? await Promise.all([
        client.from("growth_goal_versions").select("*").in("goal_id", goalIds),
        client.from("growth_goal_checkpoints").select("*").in("goal_id", goalIds),
        client.from("growth_goal_updates").select("*").in("goal_id", goalIds).order("submitted_at", { ascending: false }),
        client.from("growth_goal_decisions").select("*").in("goal_id", goalIds).order("created_at", { ascending: false }),
      ])
  ...
  const versions = new Map(
    (versionsResult.data || []).map((row) => [`${row.goal_id}:${row.version}`, row]),
  );
  ...
  const sessionsResult = relationshipIds.length
    ? await client
        .from("growth_coaching_sessions")
        .select("*")
  ...
  const [agendaResult, notesResult, confirmationsResult, actionsResult, attachmentsResult, calendarsResult] = sessionIds.length
    ? await Promise.all([
        client.from("growth_coaching_agenda_items").select("*").in("session_id", sessionIds).order("item_order"),
        client.from("growth_coaching_notes").select("*").in("session_id", sessionIds),
        client.from("growth_coaching_confirmations").select("*").in("session_id", sessionIds),
        client.from("growth_coaching_actions").select("*").in("session_id", sessionIds).order("due_date"),
        client.from("growth_attachments").select("*").in("session_id", sessionIds).eq("upload_status", "complete"),
        client.from("growth_calendar_sync").select("*").in("session_id", sessionIds).order("updated_at", { ascending: false }),
      ])
  ...
  const serializedGoals = goals.map((goal) => serializeGoal(
    goal,
  ...
  const directReports = reportEmployeeIds.map((userId) => {
    ...
    return {
      ...
      relationshipRole: reportAssignment?.primary_manager_id === profile.id
        ? "Primary manager"
        : reportAssignment?.secondary_manager_id === profile.id
          ? "Secondary manager"
          : "Higher manager",
  ...
  return {
    ...
    primaryManager: assignment?.primary_manager_id
      ? userShape(users.get(assignment.primary_manager_id) || { id: assignment.primary_manager_id, active: false })
      : null,
    secondaryManager: assignment?.secondary_manager_id
      ? userShape(users.get(assignment.secondary_manager_id) || { id: assignment.secondary_manager_id, active: false })
      : null,
    reportingLine: {
    ...
    directReports,
    plans: (plansResult.data || []).filter((row) => row.employee_id === profile.id).map((row) => ({
      id: row.id,
      employeeId: row.employee_id,
      title: row.title,
    ...
    relationships: relationships.map((row) => serializeRelationship(
      row,
      users,
      calendarByRelationship,
      profile.id,
    )),
    sessions: sessions.map((session) => {
      ...
      const privatePrep = (notesResult.data || []).find(
        (row) => row.session_id === session.id && row.note_type === "private_preparation" && row.author_id === profile.id,
      );
      return {
        id: session.id,
        ...
        content: {
          ...
          sharedNotes: session.shared_notes,
        ...
        actions: (actionsResult.data || [])
          ...
          .map((row) => ({
            ...
            canEdit: row.owner_id === profile.id,
          })),
...
export async function growthPlanSave(body = {}, accessContext) {
  ...
  const periodType = ["annual", "half_yearly", "custom"].includes(body.periodType)
    ? body.periodType
    : "custom";
  ...
  if (Number(current.revision) !== revision(body.expectedRevision)) {
    throw appError("The development plan changed after it was opened.", 409, { current });
  }
  const { data, error } = await client
    .from("growth_development_plans")
    .update({
...
export async function growthGoalSubmit(body = {}, accessContext) {
  ...
  if (Number(goal.revision) !== revision(body.expectedRevision)) {
    throw appError("The goal changed after it was opened.", 409, { current: goal });
  }
  const now = new Date().toISOString();
  const { data, error } = await client
    .from("growth_goals")
    .update({ status: "Pending Approval", revision: goal.revision + 1, updated_by: profile.id, updated_at: now })
    .eq("id", goal.id)
  ...
  await client
    ...
    .eq("goal_id", goal.id)
    .eq("version", goal.active_version);
  const { data: manager } = await client
    .from("user_profiles")
    .select("id,email,full_name")
    .eq("id", goal.primary_manager_id)
    .eq("active", true)
    .maybeSingle();
  await notify(client, {
    actor: profile,
...
export async function growthGoalProgressSave(body = {}, accessContext) {
  ...
  await eventRow(client, {
    ...
    metadata: { progress, hasEvidence: Boolean(text(body.evidence)), hasCheckpoint: Boolean(checkpointId) },
  });
  return data;
}
...
export async function coachingRelationshipInvite(body = {}, accessContext) {
  ...
  const cadence = ["weekly", "fortnightly", "monthly", "custom"].includes(body.cadence)
    ? body.cadence
    : "fortnightly";
...
async function calendarEventForSession(client, relationship, session) {
  ...
  const attendeeId = relationship.calendar_owner_id === relationship.participant_one_id
    ? relationship.participant_two_id
    : relationship.participant_one_id;
...
async function synchronizeSessionCalendar(client, relationship, session, { force = false } = {}) {
  ...
  let { data: sync, error: syncError } = await client
  ...
  if (!growthCalendarConfigured()) {
    const payload = { ...base, status: "Unavailable", last_error_code: "OUTLOOK_NOT_CONFIGURED" };
    const result = sync
      ? await client.from("growth_calendar_sync").update({ ...payload, revision: sync.revision + 1, updated_at: new Date().toISOString() }).eq("id", sync.id).select("*").single()
  ...
  try {
    ...
    const payload = {
      ...
      outlook_schedule: { start: graphEvent?.start || null, end: graphEvent?.end || null },
    ...
    const status = error.code === "OUTLOOK_EVENT_CONFLICT"
      ? "Conflict"
      : ["OUTLOOK_NOT_CONFIGURED", "EXTERNAL_ACTION_GATE_DISABLED"].includes(error.code)
        ? "Unavailable"
        : "Failed";
    const payload = { ...base, status, last_error_code: error.code || "OUTLOOK_CALENDAR_FAILED" };
    const result = sync
      ? await client.from("growth_calendar_sync").update({ ...payload, revision: sync.revision + 1, updated_at: new Date().toISOString() }).eq("id", sync.id).select("*").single()
      : await client.from("growth_calendar_sync").insert(payload).select("*").single();
    if (result.error) throw rpcError(result.error);
...
async function synchronizeSessionCalendarSafely(client, relationship, session, options = {}) {
  try {
    ...
    const status = ["OUTLOOK_NOT_CONFIGURED", "EXTERNAL_ACTION_GATE_DISABLED"].includes(error.code)
      ? "Unavailable"
      : "Failed";
    const fallback = {
      relationship_id: relationship.id,
    ...
    try {
      ...
      const result = current
        ? await client.from("growth_calendar_sync").update({
            ...fallback,
            revision: current.revision + 1,
...
export async function coachingSessionSave(body = {}, accessContext) {
  ...
  let session;
  if (body.id) {
    const context = await sessionForActor(client, id(body.id), profile.id, { activeOnly: true });
    ...
    if (Number(context.session.revision) !== revision(body.expectedRevision)) {
      throw appError("The coaching session changed after it was opened.", 409, { current: context.session });
    }
    const { data, error } = await client
      .from("growth_coaching_sessions")
      .update({
...
export async function coachingSessionCancel(body = {}, accessContext) {
  ...
  if (Number(session.revision) !== revision(body.expectedRevision)) {
    throw appError("The coaching session changed after it was opened.", 409, { current: session });
  }
...
export async function coachingSessionContentSave(body = {}, accessContext) {
  ...
  const { data, error } = await client.rpc("save_growth_coaching_session_content", {
    ...
    p_payload: rpcContentType === "agenda"
  ...
  await eventRow(client, {
    ...
    actor: profile,
    summary: rpcContentType === "addendum"
      ? "Append-only session correction added."
      : "Coaching session content updated.",
    metadata: {
...
export async function coachingSessionConfirm(body = {}, accessContext) {
  ...
  if (error) throw rpcError(error);
  if (!data?.locked) {
    const otherId = relationship.participant_one_id === profile.id
      ? relationship.participant_two_id
      : relationship.participant_one_id;
...
export async function coachingActionSave(body = {}, accessContext) {
  ...
  const status = ["To Do", "In Progress", "Blocked", "Done", "Cancelled"].includes(body.status)
    ? body.status
    : "To Do";
  ...
  if (Number(current.revision) !== revision(body.expectedRevision)) {
    throw appError("The coaching action changed after it was opened.", 409, { current });
  }
  const { data, error } = await client
    .from("growth_coaching_actions")
    .update({
...
async function attachmentForActor(client, attachmentId, profile, { write = false } = {}) {
  ...
  await attachmentScope(
    client,
    { sessionId: data.session_id, goalId: data.goal_id },
    profile,
    { write },
  );
  return data;
...
export async function growthAttachmentComplete(body = {}, accessContext) {
  const { client, profile } = accessContext;
  const attachment = await attachmentForActor(
    client,
  ...
  if (infoError || Number(info?.size || 0) !== Number(attachment.content_size)) {
    await client.storage.from(BUCKET).remove([attachment.storage_path]).catch(() => null);
...
export async function growthEmailPreferencesSave(body = {}, accessContext) {
  ...
  if (Number(current?.revision || 0) !== revision(body.expectedRevision)) {
    throw appError("Email preferences changed after they were opened.", 409, { current });
  }
  const values = Object.fromEntries([...EMAIL_CATEGORIES].map((key) => [key, preferences[key] !== false]));
  const result = current
    ? await client.from("growth_email_preferences").update({
        ...values,
        revision: current.revision + 1,
        updated_at: new Date().toISOString(),
      }).eq("user_id", profile.id).eq("revision", current.revision).select("*").single()
    : await client.from("growth_email_preferences").insert({
        user_id: profile.id,
        ...values,
      }).select("*").single();
  if (result.error) throw rpcError(result.error);
...
export async function coachingCalendarRetry(body = {}, accessContext) {
  ...
  return { calendar: results, status: results.some((row) => row.status === "Failed") ? "Failed" : results[0]?.status || "Pending" };
}

export async function coachingCalendarResolve(body = {}, accessContext) {
  const { client, profile } = accessContext;
  const relationship = await relationshipForActor(client, id(body.relationshipId), profile.id, { activeOnly: true });
  const { data: sync, error } = await client
    .from("growth_calendar_sync")
    .select("*")
    .eq("relationship_id", relationship.id)
    .eq("status", "Conflict")
  ...
  const remote = await growthCalendarGet({ organizerEmail: sync.organizer_email, eventId: sync.outlook_event_id });
...
export async function growthCoachingDailyMaintenance(client) {
  ...
  if (stalePaths?.length) {
    await client.storage.from(BUCKET).remove(stalePaths).catch(() => null);
  }
