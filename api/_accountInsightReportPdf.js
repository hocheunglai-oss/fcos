...
function table(context, headers, rows, widths, aligns = []) {
  ...
  const drawHeader = () => {
    context.doc.setFont('helvetica', 'bold'); context.doc.setFontSize(6.7);
...
function renderSection(context, section) {
  ...
  if (section.id === 'statement') { if (context.size.orientation !== 'landscape') addPage(context, true); sectionTitle(context, 'Statement', section.direction ? `Direction: ${section.direction}. Currency: ${section.currency || 'separate'}.` : 'Party-facing statement values.'); table(context, ['STEM', 'Delivery', 'Invoice state', 'Currency', 'Invoice', ...(section.includeExpected ? ['Expected'] : []), 'Paid / received', 'Balance', 'Due date'], (section.rows || []).map((row) => [row.stem, row.date, row.invoiceState, row.currency, money(row.invoice, row.currency), ...(section.includeExpected ? [money(row.expectedAmount, row.currency)] : []), money(row.paid ?? row.payments, row.currency), money(row.balance, row.currency), row.dueDate]), section.includeExpected ? [35, 28, 38, 22, 38, 38, 38, 38, 22] : [42, 32, 42, 24, 42, 42, 42, 25], ['left', 'left', 'left', 'left', 'right', 'right', 'right', 'right', 'left']); if (section.documents?.length) { sectionTitle(context, 'Invoice and credit note evidence', 'Issued, non-proforma, non-deprecated own-direction documents.'); table(context, ['STEM', 'Document number', 'Type', 'Document date', 'Currency', 'Amount'], section.documents.map((row) => [row.stem, row.number, row.type, row.date, row.currency, money(row.amount, row.currency)]), [42, 62, 31, 35, 24, 42], ['left', 'left', 'left', 'left', 'left', 'right']); } return; }
