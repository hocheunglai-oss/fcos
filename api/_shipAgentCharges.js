...
function httpError(message, status = 400, code = 'SHIP_AGENT_CHARGE_ERROR', details) {
...
function isShipAgentAccount(account) {
  return text(account?.Imported_Particulars__c, 20_000).toLowerCase().includes(SHIP_AGENT_MARKER);
}
...
function lineFingerprint(row) {
  return {
    ...
    uom: row.Unit_of_Measure__c || null, cost: row.Cost_Per_Unit__c ?? null,
    price: row.Price_Per_Unit__c ?? null, totalCost: row.Total_Cost__c ?? null,
...
function liveFingerprint(liveCase) {
  return sha256({
    ...
    accounts: liveCase.accounts.map((row) => ({ id: row.Id, particulars: row.Imported_Particulars__c || null, paymentTerm: row.Supplier_Payment_Term__c || null })).sort((a, b) => a.id.localeCompare(b.id)),
...
async function loadLiveCases({ client, stemIds = null, stemAccessCondition = null }) {
  ...
  const relevantExtras = allExtraCosts.filter((row) => shipAgentAccountIds.has(row.Supplier__c));
  ...
  const result = stemRows.map((stem) => {
    const lineItems = allLineItems.filter((row) => row.STEM__c === stem.Id && shipAgentAccountIds.has(row.Original_Supplier__c));
    const extraCosts = allExtraCosts.filter((row) => row.STEM__c === stem.Id && shipAgentAccountIds.has(row.Supplier__c));
...
function deriveStatus(live, stored, today = hongKongToday()) {
  ...
  const confirmed = live.stem.Ship_Agent_Charges_Confirmed__c === true
  ...
  if (!live.hasShipAgent) return 'completed';
...
function serializeLiveRow(row, kind) {
  ...
  return {
    ...
    cost: kind === 'line_item' ? row.Cost_Per_Unit__c ?? null : row.Unit_Cost__c ?? null,
    price: kind === 'line_item' ? row.Price_Per_Unit__c ?? null : row.Unit_Price__c ?? null,
...
function serializeCase(live, stored, profile, gm, dueDate) {
  ...
  return {
    ...
    confirmed: live.stem.Ship_Agent_Charges_Confirmed__c === true,
    ...
    capabilities: capabilitiesFor(stored || {
...
async function storedCases(client, stemIds = null) {
  let query = client.from('ship_agent_charge_cases').select(CASE_SELECT);
  if (stemIds?.length) query = query.in('stem_id', stemIds);
  const { data, error } = await query;
  if (error) throw httpError('Ship-Agent Charges storage is unavailable. Apply the required Supabase migration.', 503, 'SHIP_AGENT_STORAGE_UNAVAILABLE');
  return data || [];
}
...
export async function listShipAgentCharges(body, context) {
...
async function liveCaseForStem(stemId, context) {
  ...
  if (!rows.length) throw httpError('No Ship Agent supplier is currently detected for this STEM.', 404, 'SHIP_AGENT_CASE_NOT_FOUND');
  return rows[0];
}
...
export async function shipAgentChargeOptions(_body, context) {
...
export async function getShipAgentChargeDetail(body, context) {
  ...
  const [{ cases }, files, options] = await Promise.all([
    ...
    shipAgentChargeOptions({}, context),
  ]);
  return {
    case: cases[0],
...
async function reserveOperation(client, { operationId, type, stemId, fingerprint, actorId }) {
  const { data, error } = await client.rpc('reserve_ship_agent_charge_operation', {
...
async function completeOperation(client, operationId, status, result) {
  const { error } = await client.rpc('complete_ship_agent_charge_operation', {
    p_operation_id: operationId,
    p_status: status,
    p_result: result || {},
  });
...
function currentCaseRow(rows, stemId) {
  ...
  if (!row) throw httpError('The Ship-Agent Charge case is not synchronized yet. Refresh Payment Collections.', 409, 'CASE_NOT_SYNCHRONIZED');
  return row;
}
...
async function validateReviews(body, live, files) {
  ...
  for (const id of existingIds) {
    ...
    if (!review || review.reviewed !== true) throw httpError('Every current Ship Agent row must be reviewed individually.', 400, 'ROW_REVIEW_REQUIRED');
...
function expectedRevision(body, stored) {
  ...
  if (!Number.isInteger(expected) || expected !== Number(stored.revision || 0)) {
    throw httpError('This Ship-Agent Charge case changed after it was opened. Refresh and review it again.', 409, 'CASE_REVISION_CONFLICT', { current: { revision: Number(stored.revision || 0) } });
  }
...
function findExtra(live, id, lastModifiedDate) {
  ...
  if (!row) throw httpError('A Ship Agent extra-cost row changed or is no longer active.', 409, 'EXTRA_COST_CHANGED');
  if (!lastModifiedDate || row.LastModifiedDate !== lastModifiedDate) throw httpError('A Ship Agent extra-cost row changed after it was opened.', 409, 'EXTRA_COST_CONFLICT');
  return row;
}
...
async function salesforceChargeWrites(body, live) {
  ...
  const supplierIds = new Set(live.accounts.map((row) => row.Id));
  const requests = [];
  ...
  for (const update of updates) {
    ...
    const patch = {};
    patch.Description__c = text(update.description, 32_000) || null;
    const paymentTerm = text(update.paymentTerm, 255);
    if (paymentTerm) patch.Payment_Term__c = paymentTerm;
    if (mode === 'fixed') {
      patch.Lumpsum_Cost__c = numeric(update.supplierCost ?? update.cost ?? update.fixedAmount, 'Fixed cost');
      patch.Lumpsum_Price__c = numeric(update.buyerPrice ?? update.price ?? update.fixedAmount, 'Fixed buyer price');
      patch.Unit_Cost__c = null; patch.Unit_Price__c = null;
    } else {
      patch.Quantity__c = numeric(update.quantity, 'Quantity', { positive: true, nullable: false });
      patch.Unit_of_Measure__c = text(update.unitOfMeasure || current.Unit_of_Measure__c, 40) || '1.';
      patch.Unit_Cost__c = numeric(update.supplierCost ?? update.cost ?? update.unitPrice, 'Unit cost');
      patch.Unit_Price__c = numeric(update.buyerPrice ?? update.price ?? update.unitPrice, 'Unit buyer price');
      patch.Lumpsum_Cost__c = null; patch.Lumpsum_Price__c = null;
    }
  ...
  for (const cancel of cancellations) {
    ...
    const lastModifiedDate = text(typeof cancel === 'string' ? '' : cancel?.expectedLastModifiedDate || cancel?.lastModifiedDate, 80);
    const current = findExtra(live, id, lastModifiedDate);
    requests.push({ method: 'PATCH', url: `/services/data/${apiVersion}/sobjects/STEM_Extra_Cost__c/${id}`, referenceId: `cancel${reference++}`, httpHeaders: lastModifiedHeaders(current.LastModifiedDate), body: { Cancelled__c: true } });
  }
  for (const addition of additions) {
    const supplierId = text(addition?.supplierAccountId || addition?.supplierId, 18);
    const productId = text(addition?.productId, 18);
    if (!supplierIds.has(supplierId)) throw httpError('New STEM Charges must use the exact Ship Agent supplier Account on this STEM.', 400, 'SUPPLIER_MISMATCH');
    ...
    const accountPaymentTerm = text(account?.Supplier_Payment_Term__c, 255);
    const childPaymentTerms = [...new Set([
      ...live.lineItems.filter((row) => row.Original_Supplier__c === supplierId).map((row) => text(row.Payment_Term__c, 255)),
      ...live.extraCosts.filter((row) => row.Supplier__c === supplierId).map((row) => text(row.Payment_Term__c, 255)),
    ].filter(Boolean))];
    const inheritedPaymentTerm = accountPaymentTerm || (childPaymentTerms.length === 1 ? childPaymentTerms[0] : null);
    if (!inheritedPaymentTerm) throw httpError('The Ship Agent supplier payment term is unavailable or ambiguous and cannot be guessed.', 409, 'PAYMENT_TERM_UNAVAILABLE');
    const mode = (addition.pricingType || addition.pricingMode) === 'per_unit' ? 'per_unit' : 'fixed';
    const create = {
      STEM__c: live.stem.Id, Supplier__c: supplierId, Product2Id__c: productId,
      RecordTypeId: recordTypes[0].Id, Payment_Term__c: inheritedPaymentTerm, Cancelled__c: false,
      Description__c: text(addition.description, 32_000) || 'STEM Charge',
    };
    if (mode === 'fixed') {
      create.Lumpsum_Cost__c = numeric(addition.supplierCost ?? addition.cost ?? addition.fixedAmount, 'Fixed cost');
      create.Lumpsum_Price__c = numeric(addition.buyerPrice ?? addition.price ?? addition.fixedAmount, 'Fixed buyer price');
      ...
      create.Unit_of_Measure__c = text(addition.unitOfMeasure, 40) || '1.';
      create.Unit_Cost__c = numeric(addition.supplierCost ?? addition.cost ?? addition.unitPrice, 'Unit cost');
      create.Unit_Price__c = numeric(addition.buyerPrice ?? addition.price ?? addition.unitPrice, 'Unit buyer price');
    }
    requests.push({ method: 'POST', url: `/services/data/${apiVersion}/sobjects/STEM_Extra_Cost__c`, referenceId: `create${reference++}`, body: create });
  }
  ...
  const result = await sfRequest('/composite', {
    ...
    body: {
      ...
      compositeRequest: [{
        ...
        body: { Ship_Agent_Charges_Confirmed__c: confirmed === true },
      }],
    },
  });
  const response = result?.compositeResponse?.[0];
  if (!response || response.httpStatusCode < 200 || response.httpStatusCode >= 300) {
    throw httpError(response?.body?.[0]?.message || 'Salesforce could not record the Ship Agent confirmation.', 502, 'SALESFORCE_CONFIRMATION_FAILED');
  }
}

export async function saveAndConfirmShipAgentCharges(body, context) {
  ...
  if (!liveBefore.stem.Delivery_Date__c || hongKongToday() <= liveBefore.stem.Delivery_Date__c) throw httpError('Ship Agent charges become actionable after the Salesforce Delivery Date.', 409, 'DELIVERY_NOT_COMPLETE');
  let reviews;
  ...
  try {
    ...
    const { data, error } = await context.client.rpc('confirm_ship_agent_charge_case', {
      p_stem_id: stemId,
      ...
      p_event: {
        ...
        summary: 'Ship-Agent charges confirmed.',
        metadata: {
...
export async function overrideShipAgentChargeAssignment(body, context) {
  const stemId = text(body?.stemId, 18);
  const operationId = operationIdentity(body);
  ...
  const gm = await activeGeneralManager(context.client, context.profile.id);
  if (!gm.isGeneralManager) throw httpError('Only the active General Manager may temporarily reassign a Ship-Agent Charge case.', 403, 'GENERAL_MANAGER_REQUIRED');
  const stored = currentCaseRow(await storedCases(context.client, [stemId]), stemId);
  ...
  const { data, error } = await context.client.rpc('override_ship_agent_charge_assignment', {
...
export async function resolveShipAgentPostInvoiceChange(body, context) {
  const stemId = text(body?.stemId, 18);
  const operationId = operationIdentity(body);
  ...
  const { data, error } = await context.client.rpc('resolve_ship_agent_post_invoice_change', {
...
function syncPayload(live, stored, dueDate) {
  ...
  return {
    ...
    supplierFingerprint: sha256(live.accounts.map((row) => ({ id: row.Id, particulars: row.Imported_Particulars__c || null, paymentTerm: row.Supplier_Payment_Term__c || null }))),
    salesforceStemLastModifiedAt: live.stem.LastModifiedDate || null,
...
export async function syncShipAgentCharges(context, { stemIds = null } = {}) {
  ...
  for (const entry of live) {
    ...
    const { data, error } = await context.client.rpc('sync_ship_agent_charge_case', {
      p_case: payload,
      p_event: {
        eventType: 'synced',
        eventKey: `sync:${payload.stemId}:${payload.sourceFingerprint}:${payload.workflowStatus}`,
        summary: 'Ship-Agent case synchronized from live Salesforce data.',
        metadata: {
...
export const shipAgentChargeInternals = {
  deriveStatus,
  effectiveAssignee,
  finalInvoice,
  isShipAgentAccount,
