...
export function serializeFinanceSettings(row) {
  ...
  return { annualInterestRatePct: validateAnnualInterestRate(row.annual_interest_rate_pct),
    revision: Number(row.revision), updatedAt: row.updated_at || null, updatedByEmail: row.updated_by_email || null };
}

export async function loadFinanceSettings(client) {
  const result = await client.from('company_finance_settings')
    .select('annual_interest_rate_pct,revision,updated_at,updated_by_email').eq('setting_key', 'company').maybeSingle();
...
export function createFinanceSettingsHandlers({ requireActiveUser, userHasAnyModuleAccess, userHasCapability, expireCache }) {
  ...
  async function financeSettingsSave(body = {}, req = null, accessContext = null) {
    ...
    const result = await context.client.rpc('save_company_finance_settings', {
      p_annual_interest_rate_pct: rate, p_expected_revision: body.expectedRevision, p_actor_user_id: context.profile.id,
    });
    if (result.error) {
      ...
      throw financeError('Finance settings could not be saved. Refresh to check the current rate before retrying.', 503, 'FINANCE_SETTINGS_SAVE_FAILED');
    }
